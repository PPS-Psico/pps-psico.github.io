...
function TopBar({ user = 'Luis Battaglia', active = 'lanzador' }) {
  // Tabs that have a real page navigate; the rest are inert (coming soon)
  const LINKS = { inicio: 'Inicio.html', lanzador: 'Lanzador.html', gestion: 'Gestion.html', solicitudes: 'Solicitudes.html', metricas: 'Metricas.html' };
  const go = (k) => { if (LINKS[k] && k !== active) window.location.href = LINKS[k]; };
  return (
    <header style={{
      ...
      <div className="topbar-inner" style={{ maxWidth: '100%', margin: 0, padding: '14px 24px', display: 'flex', alignItems: 'center', gap: 20, minWidth: 0 }}>
        ...
        <nav className="topbar-tabs" style={{ display: 'flex', gap: 2, marginLeft: 16, flexShrink: 0 }}>
          {[
            ...
            <button key={t.k} onClick={() => go(t.k)} className="press" style={{
              border: 'none', background: 'transparent', cursor: LINKS[t.k] && t.k !== active ? 'pointer' : 'default',
