...
-- La desaprobaciÃ³n de una PPS es una decisiÃ³n excepcional de la instituciÃ³n.
-- La prÃ¡ctica se conserva como antecedente, pero no computa horas ni rotaciones.
...
comment on column public.practicas.desaprobacion_causas is
  'Causas canÃ³nicas: inasistencia_responsabilidad y/o falta_participacion_actitud.';
comment on column public.practicas.desaprobacion_motivo_publico is
  'ExplicaciÃ³n visible para el estudiante. No incluir datos sensibles ni referencias internas.';
...
comment on function public.practica_computa(text) is
  'Regla Ãºnica para horas, especialidad y rotaciones: las PPS desaprobadas o no concretadas no computan.';

-- El trigger legado revertÃ­a el estado cuando una migraciÃ³n o tarea de servicio
-- no tenÃ­a auth.uid(). Mantiene la protecciÃ³n estudiantil, pero permite tareas
...
-- VinculaciÃ³n y ciclo de vida de las penalizaciones.
...
-- Los tres registros heredados completamente vacÃ­os se conservan para auditorÃ­a,
-- pero quedan anulados y fuera de cualquier cÃ¡lculo.
update public.penalizaciones
set tipo_incumplimiento = coalesce(tipo_incumplimiento, 'Registro invÃ¡lido heredado'),
    puntaje_penalizacion = coalesce(puntaje_penalizacion, 0),
    estado = 'Anulada',
    anulada_at = coalesce(created_at, now()),
    anulacion_motivo = 'Registro heredado sin estudiante ni datos de penalizaciÃ³n'
where estudiante_id is null
  and tipo_incumplimiento is null
  and puntaje_penalizacion is null;

-- Antes de normalizar, se registra cada correcciÃ³n de puntaje con su valor anterior.
...
select
  'MigraciÃ³n canÃ³nica de penalizaciones',
  'penalty_score_normalized',
  'penalizaciones',
  p.id::text,
  'Puntaje histÃ³rico normalizado al criterio institucional vigente',
  jsonb_build_object(
    ...
    'puntaje_nuevo', case p.tipo_incumplimiento
      ...
      when 'Ausencia en Inicio / No se presentÃ³' then 50
      when 'Abandono durante la PPS' then 70
      when 'Falta sin Aviso' then 40
      when 'Baja Administrativa / Sin PenalizaciÃ³n' then 0
...
set tipo_incumplimiento = case
      when tipo_incumplimiento = 'Ausencia en Inicio / No se presentÃ³'
...
create unique index if not exists uq_penalizacion_desaprobacion_activa_por_practica
  on public.penalizaciones (practica_id)
  where estado = 'Activa'
    and tipo_incumplimiento = 'PPS desaprobada por la instituciÃ³n';

-- El estudiante puede seguir corrigiendo sus registros ordinarios, pero no puede
-- alterar ni borrar una decisiÃ³n institucional. Una desaprobaciÃ³n se corrige por RPC.
...
begin
  ...
  if tg_op = 'UPDATE'
    ...
    raise exception 'La desaprobaciÃ³n de una PPS sÃ³lo puede ser gestionada por coordinaciÃ³n.'
      using errcode = '42501';
  end if;
...
-- La deselecciÃ³n automÃ¡tica no debe borrar un antecedente ya desaprobado.
do $$
declare
  v_definition text;
  v_patched text;
begin
  ...
  if v_patched = v_definition then
    raise exception 'No se pudo proteger la baja automÃ¡tica de prÃ¡cticas desaprobadas';
  end if;
...
begin
  if not public.is_admin() then
    raise exception 'Acceso restringido a coordinaciÃ³n'
      using errcode = '42501';
  end if;

  if p_fecha is null or p_notificado_at is null then
    raise exception 'La fecha de desaprobaciÃ³n y la notificaciÃ³n son obligatorias'
      using errcode = '22023';
  end if;
  ...
  if coalesce(array_length(p_causas, 1), 0) = 0
    ...
    raise exception 'Debe seleccionarse al menos una causa vÃ¡lida'
      using errcode = '22023';
  end if;
  ...
  if not found then
    raise exception 'PrÃ¡ctica inexistente'
      using errcode = 'P0002';
  end if;

  if v_practica.tipo_actividad <> 'pps' then
    raise exception 'La desaprobaciÃ³n institucional sÃ³lo aplica a PPS'
      using errcode = '22023';
  end if;
  ...
  insert into public.penalizaciones (
    ...
    'PPS desaprobada por la instituciÃ³n',
    p_fecha::text,
  ...
  insert into public.admin_action_log (
    ...
    'practicas',
    p_practica_id::text,
    'PPS marcada como desaprobada por decisiÃ³n institucional',
    jsonb_build_object(
...
begin
  ...
  if p_nuevo_estado not in ('En curso', 'Finalizada', 'Convenio Realizado', 'No se pudo concretar')
    ...
    raise exception 'Estado de correcciÃ³n o motivo invÃ¡lido'
      using errcode = '22023';
  end if;
  ...
  if not found or v_practica.estado <> 'Desaprobada' then
    raise exception 'La prÃ¡ctica no estÃ¡ desaprobada'
      using errcode = '22023';
  end if;
  ...
  insert into public.admin_action_log (
    ...
    'practicas',
    p_practica_id::text,
    'DesaprobaciÃ³n anulada mediante correcciÃ³n administrativa',
    jsonb_build_object(
      'estudiante_id', v_practica.estudiante_id,
      'lanzamiento_id', v_practica.lanzamiento_id,
...
-- ReconstrucciÃ³n documental: sÃ³lo los siete casos confirmados por comunicaciÃ³n
...
insert into _pps_disapproval_backfill values
  (
    ...
    'Incumplimiento del mÃ­nimo de asistencia y ausencias sin aviso en los espacios de supervisiÃ³n.',
  ...
  (
    ...
    'Incumplimiento del mÃ­nimo de asistencia y evaluaciÃ³n institucional desfavorable del compromiso y la participaciÃ³n.',
  ...
  (
    ...
    date '2026-04-08',
    array['inasistencia_responsabilidad', 'falta_participacion_actitud'],
    'Incumplimiento del mÃ­nimo de asistencia y evaluaciÃ³n institucional desfavorable de la responsabilidad y la participaciÃ³n.',
  ...
  (
    ...
    'La instituciÃ³n informÃ³ una evaluaciÃ³n desfavorable del desempeÃ±o, el compromiso y la adecuaciÃ³n al encuadre profesional.',
  ...
  (
    ...
    date '2025-11-26',
    array['falta_participacion_actitud'],
    'La instituciÃ³n dispuso la baja por incumplimientos graves y reiterados del encuadre Ã©tico-profesional.',
  ...
  (
    ...
    date '2026-07-20',
    array['falta_participacion_actitud'],
    'La instituciÃ³n informÃ³ una evaluaciÃ³n desfavorable de la participaciÃ³n activa, el compromiso con las tareas y el posicionamiento profesional.',
...
-- Dos prÃ¡cticas habÃ­an desaparecido por el flujo histÃ³rico de deselecciÃ³n.
...
with resolved as (
  ...
  join public.penalizaciones as pen
    ...
    and pen.tipo_incumplimiento = 'PPS desaprobada por la instituciÃ³n'
  order by e.id
)
insert into public.admin_action_log (
  actor_name,
...
select
  'ReconstrucciÃ³n documental Gmail',
  'pps_disapproval_backfilled',
  'practicas',
  r.practica_id::text,
  'DesaprobaciÃ³n institucional reconstruida desde comunicaciones histÃ³ricas',
  jsonb_build_object(
...
-- Los reportes agregados vigentes pasan a usar la misma regla de cÃ³mputo.
-- Se parchean definiciones ya desplegadas y se aborta la migraciÃ³n si alguna
-- firma cambiÃ³, para evitar una correcciÃ³n silenciosamente incompleta.
do $$
declare
...
begin
  foreach v_signature in array array[
    ...
    if v_patched = v_definition then
      raise exception 'No se pudo actualizar el cÃ³mputo de %', v_signature;
    end if;
...
$replacement$
  ...
  if v_patched = v_definition then
    raise exception 'No se pudo actualizar el cÃ³mputo de private.get_interview_completion_candidates_v1_impl()';
  end if;
