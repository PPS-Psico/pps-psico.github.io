...
-- La migración 20260902120000 abrió el catálogo a las tareas que ninguna
-- jefatura leyó nunca. El alcance era correcto pero el ritmo no: pasó el
-- catálogo de laboral+comunitaria de ~15 a 42 tareas de una vez.
--
-- El barrido no corre en un servidor: corre en el navegador de la jefatura,
-- dentro de la Etiqueta del curso, y parsea cada tabla de calificación con
-- DOMParser en el hilo principal. Medido hoy en producción: una tabla de esas
-- pesa ~420 KB y devuelve 100 filas por página, así que la etiqueta entra a
-- paginar -- hasta 24 páginas por tarea. Con 42 tareas la pestaña se cuelga
-- antes de completar un solo lote: 15 minutos de barrido, cero lotes escritos,
-- y el navegador sin responder.
--
-- Se toman de a seis. El orden es por cmid, que es estable dentro de una
-- corrida: una tarea sólo sale del conjunto cuando ya fue leída, y las que
-- quedan conservan su lugar, así que ningún lote en vuelo puede quedar apuntando
-- a un cmid que el catálogo ya no acepta. Cada barrido completo drena seis y el
-- siguiente toma las seis que siguen; las 51 pendientes salen en nueve pasadas.
...
comment on function private.get_jefe_moodle_sync_tasks_for_areas_v1(text[]) is
  'Catálogo de tareas Moodle escaneables por jefatura. Incluye tareas de años previos con entregas abiertas, con entregas que constan pero siguen sin fecha verificable, y hasta seis tareas por barrido con prácticas terminadas sin corregir que ninguna jefatura leyó nunca.';
