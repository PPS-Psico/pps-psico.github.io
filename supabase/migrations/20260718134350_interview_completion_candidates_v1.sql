...
-- de proximidad a la finalizaciÃ³n y excluye a quienes ya poseen una prÃ¡ctica
-- de Relevamiento del Ejercicio Profesional o Entrevista a Profesionales.
-- La coincidencia es deliberadamente especÃ­fica para no confundir estas PPS
-- con Entrevistas de AdmisiÃ³n u otras actividades de entrevista.
...
begin
  ...
  with normalized_practices as (
    select
      p.*,
      case
        when translate(lower(trim(coalesce(p.especialidad, ''))), 'Ã¡Ã©Ã­Ã³ÃºÃ¼Ã±', 'aeiouun')
          in ('clinica', 'laboral', 'educacional', 'comunitaria')
          then translate(lower(trim(p.especialidad)), 'Ã¡Ã©Ã­Ã³ÃºÃ¼Ã±', 'aeiouun')
        when translate(lower(coalesce(p.especialidad, '')), 'Ã¡Ã©Ã­Ã³ÃºÃ¼Ã±', 'aeiouun')
          like '%[educacional%'
          then 'educacional'
        else null
      end as orientation_key,
      case
        when translate(lower(trim(coalesce(p.especialidad, ''))), 'Ã¡Ã©Ã­Ã³ÃºÃ¼Ã±', 'aeiouun') = 'clinica'
          then 'ClÃ­nica'
        when translate(lower(trim(coalesce(p.especialidad, ''))), 'Ã¡Ã©Ã­Ã³ÃºÃ¼Ã±', 'aeiouun') = 'laboral'
          then 'Laboral'
        when translate(lower(trim(coalesce(p.especialidad, ''))), 'Ã¡Ã©Ã­Ã³ÃºÃ¼Ã±', 'aeiouun') = 'educacional'
          then 'Educacional'
        when translate(lower(trim(coalesce(p.especialidad, ''))), 'Ã¡Ã©Ã­Ã³ÃºÃ¼Ã±', 'aeiouun') = 'comunitaria'
          then 'Comunitaria'
        when translate(lower(coalesce(p.especialidad, '')), 'Ã¡Ã©Ã­Ã³ÃºÃ¼Ã±', 'aeiouun')
          like '%[educacional%'
          then 'Educacional'
  ...
  practice_rollup as (
    select
      ...
      count(*) filter (
        where translate(lower(trim(coalesce(p.estado, ''))), 'Ã¡Ã©Ã­Ã³ÃºÃ¼Ã±', 'aeiouun')
  ...
  classified as (
    select
      ...
      case
        ...
        when pr.total_hours >= v_total_target and pr.rotations = v_rotations_target - 1
          then 'AlcanzÃ³ 250 horas y le falta una orientaciÃ³n para completar las tres'
        when pr.total_hours >= v_total_target
          and pr.rotations >= v_rotations_target
          and sr.specialty_hours >= v_specialty_target - 20
          and sr.specialty_hours < v_specialty_target
          then 'AlcanzÃ³ 250 horas y le faltan 20 horas o menos de su especialidad'
        else null
      end as reason_label
    from public.estudiantes as e
...
comment on function public.get_interview_completion_candidates_v1() is
  'Estudiantes prÃ³ximos a finalizar elegibles para una PPS de Entrevista a Profesionales; excluye quienes ya realizaron Relevamiento o Entrevista a Profesionales.';
