-- Sincronizacion masiva para jefaturas desde la tabla de entregas Moodle.
--
-- Una misma tarea se reutiliza entre relanzamientos. Por eso el alcance se
-- define por tarea unica del anio academico y orientacion de la jefatura; el
-- servidor vuelve a resolver cada fila por cmid + DNI y rechaza coincidencias
-- ambiguas en lugar de atribuirlas a una practica arbitraria.
...
begin
  ...
  return query
  with launch_tasks as (
    select
      ae.academic_year,
      ae.course_id,
      ae.moodle_id::bigint as cmid,
      coalesce(nullif(trim(ae.moodle_name), ''), nullif(trim(ae.institucion), ''), 'Tarea Moodle') as task_name,
...
begin
  ...
  if exists (
    select 1
    from jsonb_array_elements(p_tasks) task
    left join private.get_jefe_moodle_sync_tasks_v1_impl() allowed
  ...
  if found then
    if v_existing_auth_user_id is distinct from v_auth_user_id then
      raise exception 'Request id already belongs to another user' using errcode = '42501';
    end if;
  ...
  set
    ...
    details = v_result || jsonb_build_object(
      'scope', 'jefe_area_year',
      'areas', to_jsonb(v_areas)
    )
  where r.request_id = p_request_id;
...
revoke all on function private.sync_jefe_moodle_reports_v1_impl(
...
comment on function public.get_jefe_moodle_sync_tasks_v1() is
  'Lista deduplicada de tareas Moodle del anio actual y orientaciones del Jefe autenticado.';
comment on function public.sync_jefe_moodle_reports_v1(
  uuid, bigint, integer, timestamptz, bigint, text, jsonb
) is
  'Ingiere el barrido anual de entregas Moodle del Jefe. Resuelve cmid + DNI en servidor y omite coincidencias ambiguas.';
