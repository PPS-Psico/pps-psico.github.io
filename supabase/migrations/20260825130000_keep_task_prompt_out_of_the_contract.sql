...
-- Saca la consigna del contrato de aprovisionamiento.
--
-- Ver el comentario dentro de la funcion: la instruccion util vive en las
-- tareas compartidas, que son de la catedra y algunas ya tienen su propio
-- enunciado. Automatizarlo las pisaria.
...
begin
  ...
  loop
    ...
    -- La consigna la escribe la catedra, no la automatizacion.
    --
    -- El primer intento la fijaba en las tareas `dedicated`, que son las que NO
    -- tienen el problema: se crean una por PPS. La confusion vive en las
    -- compartidas -Fundacion Tiempo, Ateneos Ulloa, Asociacion PENSAR-, que son
    -- de la catedra. Y al revisarlas se vio por que no conviene automatizarlo:
    -- Ateneos Ulloa ya tiene su propia consigna ("Deben subir un informe por
    -- cada comision..."), correcta, que un contrato con `description_html`
    -- habria pisado en el siguiente reconcile.
    --
    -- Se deja en null: el enunciado no forma parte del contrato y esas pocas
    -- tareas se redactan a mano.
    v_description := null;
