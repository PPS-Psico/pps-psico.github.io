begin;
-- Preserve the historical baseline; every INSERT after this migration defaults
-- to a dedicated task, independent of academic year. No historical adoption.
alter table public.lanzamientos_pps add column moodle_task_policy text not null
  default 'legacy_shared' check (moodle_task_policy in ('legacy_shared','dedicated'));
alter table public.lanzamientos_pps alter column moodle_task_policy set default 'dedicated';
comment on column public.lanzamientos_pps.moodle_task_policy is
'Creation policy approved 2026-09-07: new launches dedicated; historical confirmed tasks preserved.';
update public.lanzamientos_pps l set moodle_task_policy='dedicated'
where coalesce(l.moodle_pilot_dedicated,false) or exists(
  select 1 from public.moodle_task_intents i where i.lanzamiento_id=l.id and i.mode='dedicated' and i.provisioning_status<>'cancelled');
  ...
  for v_unit in
    with confirmed_legacy as (
      ...
      where lm.validation_status = 'confirmed'
        ...
        and l.moodle_task_policy = 'legacy_shared'
      ...
      where (l.moodle_task_policy = 'dedicated'
...
-- Existing triggers reconcile on activation and participant insertion.
-- Policy and name edits also need reconciliation; no browser work runs here.
drop trigger reconcile_moodle_intents_after_launch_change on public.lanzamientos_pps;
create trigger reconcile_moodle_intents_after_launch_change
after insert or update of estado_convocatoria,fecha_inicio,fecha_finalizacion,orientacion,moodle_task_policy,nombre_pps
on public.lanzamientos_pps for each row execute function private.trigger_reconcile_moodle_task_intents_v1();
commit;
