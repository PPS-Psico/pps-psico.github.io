-- Los informes sin calificar dejan la cola prioritaria cuando acumulan más de
-- 60 días de atraso respecto del seguimiento interno de 30 días. Permanecen
-- visibles como antecedentes para no perder trazabilidad.
...
as $$
  with base as (
    ...
    select
      c.*,
      case
        when c.graded then 'corrected'
        when c.submitted
          and c.deadline_at is not null
          and c.deadline_at
            < (now() at time zone 'America/Argentina/Buenos_Aires')::date - 60
          then 'stale'
        when c.submitted then 'pending'
        else 'waiting'
...
revoke all on function private.jefe_report_rows_v1(text[]) from public, anon, authenticated;
grant execute on function private.jefe_report_rows_v1(text[]) to service_role;

comment on function private.jefe_report_rows_v1(text[]) is
  'Detalle canónico de informes por área. Conserva como stale los casos con más de 60 días de atraso para excluirlos de la cola prioritaria sin perder trazabilidad.';
