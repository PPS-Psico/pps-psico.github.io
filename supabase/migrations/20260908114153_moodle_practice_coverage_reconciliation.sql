-- A real report obligation remains eligible even if its administrative launch is Cerrado.
-- Historical policies and confirmed tasks are preserved. No historical auto-adoption.
begin;
    ...
    with confirmed_legacy as (
      ...
      where (l.moodle_task_policy = 'dedicated'
        ...
        and (coalesce(l.moodle_pilot_dedicated, false)
             ...
             or exists (select 1 from public.practicas obligation
               where obligation.lanzamiento_id=l.id
                 and coalesce(obligation.tipo_actividad,'pps') <> 'actividad_especial'
                 and lower(coalesce(obligation.estado,'')) not in
                   ('cancelada','cancelado','abandonada','abandonado','desaprobada','desaprobado')))
...
begin
  if not private.moodle_v2_is_coordinator() then return new; end if;
  if tg_table_name = 'lanzamientos_pps' then v_launch_id := new.id;
  else v_launch_id := new.lanzamiento_id; end if;
  -- Eligibility belongs to the reconciler, not a second administrative-state filter.
  if v_launch_id is not null then
    perform private.reconcile_moodle_task_intents_v1_impl(v_launch_id);
  end if;
  if tg_op = 'UPDATE' and tg_table_name = 'practicas' then
    if old.lanzamiento_id is distinct from new.lanzamiento_id and old.lanzamiento_id is not null then
      perform private.reconcile_moodle_task_intents_v1_impl(old.lanzamiento_id);
    end if;
  end if;
  return new;
end;
...
drop trigger reconcile_moodle_intents_after_practice_change on public.practicas;
create trigger reconcile_moodle_intents_after_practice_change
after insert or update of lanzamiento_id, estudiante_id, especialidad, estado, tipo_actividad
on public.practicas for each row
