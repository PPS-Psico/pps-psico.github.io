...
-- El sync de Moodle del jefe ya excluye a propósito una práctica vieja
-- cuando el mismo alumno tiene otra práctica "En curso" apuntando al mismo
-- cmid (ver sync_jefe_moodle_reports_scoped_v1_impl / candidate_scope): la
-- entrega real solo se guarda contra la práctica activa. Pero el listado del
-- panel de jefatura no aplicaba ese mismo criterio, así que esas prácticas
-- viejas quedaban mostradas para siempre como "pendiente, sin fecha
-- verificable" porque nunca van a recibir su propia fecha.
--
-- Este parche hace que jefe_report_rows_v1 oculte una práctica no vigente
-- ("Finalizada"/etc.) cuando exista otra práctica del mismo alumno, para el
-- mismo cmid de Moodle, que sí está "En curso" — pero solo si esa práctica
-- vieja no tiene ninguna fecha propia ya verificada. Si ya tiene una fecha
-- real guardada, se sigue mostrando normalmente.
...
comment on function private.jefe_report_rows_v1(text[]) is
  'Detalle canónico de informes por área. Usa la fecha real informada por Moodle; oculta prácticas viejas ya superadas por otra práctica En curso del mismo alumno sobre el mismo cmid cuando esa práctica vieja no tiene fecha propia.';
