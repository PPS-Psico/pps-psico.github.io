-- A partial set of verified notes must not produce an apparently final SAC
-- average. Keep each available note visible, but emit the average only when
-- every practice in the request has a numeric authoritative grade.
...
begin
  ...
  select
    ...
    r.grade_display,
    case
      when count(*) filter (where r.nota_numeric is null) over () = 0
        then round(avg(r.nota_numeric) over (), 0)
      else null
...
comment on function public.get_finalization_grade_resolution(uuid) is
  'Resuelve solo notas con procedencia Moodle/admin y calcula promedio SAC unicamente cuando todas las PPS tienen nota numerica autoritativa.';
