...
-- El barrido de jefaturas devolvía 500 en todos los lotes. El header lo decía:
-- `proxy-status: PostgREST; error=57014` -- 57014 es `query_canceled` -- con
-- `x-envoy-upstream-service-time: 8093`. Ocho segundos: el statement_timeout
-- del rol `authenticated`. No era autenticación, ni validación, ni volumen de
-- filas: la función se pasaba de tiempo y Postgres la cancelaba.
--
-- La culpa es de la migración 20260902120000. El CTE de tareas nunca leídas
-- unía `practicas` contra `aula_entregas` con las condiciones de vínculo en el
-- ON, lo que obliga a un nested loop de todas las prácticas por todas las
-- tareas, y encima resolvía "¿ya la barrió una jefatura?" con una subconsulta
-- correlacionada fila por fila. Medido: 2.737 ms por llamada. El sync invoca
-- este catálogo tres veces por request, así que cada lote llegaba al límite.
--
-- Ahora: las prácticas pendientes se filtran primero, los vínculos salen de las
-- tablas de enlace por índice, y las tareas ya barridas se calculan una sola
-- vez. Medido sobre producción: 652 ms, y devuelve exactamente el mismo
-- conjunto -- 27 tareas para laboral+comunitaria, cero diferencias en ambos
-- sentidos contra la versión anterior.
...
drop function if exists private.zz_fast_catalog(text[]);
