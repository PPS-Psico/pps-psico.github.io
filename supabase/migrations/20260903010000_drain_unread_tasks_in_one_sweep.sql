...
-- La tanda de seis existía por una razón que ya no vale. Cuando se agregaron
-- las tareas nunca leídas, el barrido se colgaba y el diagnóstico fue "es
-- demasiado volumen"; en realidad era que el catálogo tardaba 2.737 ms y el
-- RPC moría por statement_timeout (ver 20260903000000). Arreglado eso, un
-- barrido de 29 tareas corrió completo en unos dos minutos y trajo 185 filas.
--
-- Con el límite en seis, el atraso se drena a razón de seis tareas por visita
-- de cada jefatura: dos o tres pasadas por área que dependen de que alguien se
-- acuerde de entrar. Se sube a 30 para que una sola pasada por área lo termine.
-- Medido: el catálogo pasa a 611 ms (tres llamadas por request, 1,8 s contra un
-- techo de 8) y quedan 38 tareas para laboral+comunitaria, 38 para clínica y 23
-- para educacional. Sigue habiendo tope: es un barrido en el navegador de una
-- persona, no un proceso de servidor.
...
drop function if exists private.zz_all(text[]);
