-- ============================================================================
-- Coherencia de KPIs: haciendo_pps acotado al año + orientación por personas
-- ============================================================================
-- Depende de 20260601170000_add_estudiantes_en_pps_metric.sql.
--
-- Cambios:
--   1. haciendo_pps → estudiantes con práctica INICIADA en el año (mismo
--      universo que la etapa "Hicieron PPS" del embudo). Antes contaba
--      'en curso' sin filtrar por año, dando 187 vs 183 del embudo.
--   2. orientation_distribution → count(DISTINCT estudiante_id) por área, no
--      count(*) de convocatorias. Antes sumaba 917 (postulaciones) para ~212
--      personas: un alumno se postula a varias PPS de distintas áreas.
--
-- sin_pps y proximos_finalizar se dejan como SNAPSHOT (estado actual): son
-- métricas operativas "al día de hoy", el frontend las rotula así y no les
-- muestra tendencia interanual.
-- ============================================================================
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    -- haciendo_pps = estudiantes con práctica INICIADA en el año (= embudo).
    'haciendo_pps', (
      SELECT count(DISTINCT p.estudiante_id) FROM practicas p
      WHERE extract(year from safe_date_cast(p.fecha_inicio)) = p_year
    ),
    ...
    -- Distribución por área = PERSONAS distintas por orientación (no postulaciones).
      ...
      FROM (
        SELECT 
          CASE 
            ...
            ELSE 'Sin definir'
          END as orientation,
          count(DISTINCT c.estudiante_id) as cnt
...
-- ----------------------------------------------------------------------------
-- get_haciendo_pps_list: alinear con el KPI haciendo_pps (práctica iniciada en
-- el año). Antes filtraba por estado 'en curso/pendiente/en proceso' sin acotar
-- el año, dando una lista que no coincidía con el número del dashboard.
-- ----------------------------------------------------------------------------
DROP FUNCTION IF EXISTS public.get_haciendo_pps_list(int);
CREATE OR REPLACE FUNCTION public.get_haciendo_pps_list(p_year integer)
