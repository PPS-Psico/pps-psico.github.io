...
function Briefing({ data, compact, onAccept, onDismiss, isRead, onMarkRead, generatedAgo }) {
  return (
    <section data-screen-label="01 Briefing" style={{
      minHeight: isRead ? 'auto' : 'calc(100vh - 60px)',
      display: 'flex', flexDirection: 'column',
      padding: isRead ? '32px 0' : '56px 0 48px',
      borderTop: '1px solid var(--rule-2)',
      borderBottom: '1px solid var(--rule-2)',
      position: 'relative',
      transition: 'min-height .4s ease, padding .4s ease',
    }}>
      {/* Top status line — daily ritual */}
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: isRead ? 16 : 40, gap: 24, flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <span className="mono meta" style={{ borderLeft: '1px solid var(--rule-2)', paddingLeft: 14 }}>
            {data.written_at}
          </span>
          <span className="mono meta" style={{ borderLeft: '1px solid var(--rule-2)', paddingLeft: 14 }}>
            {generatedAgo}
          ...
          {isRead && (
            <span className="label" style={{ color: 'var(--ok)', borderLeft: '1px solid var(--rule-2)', paddingLeft: 14, display: 'inline-flex', alignItems: 'center', gap: 6 }}>
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          <button className="btn btn-ghost btn-sm" onClick={onDismiss} title="Briefings anteriores">
      ...
      {/* Collapsed view (after marked as read) */}
      {isRead ? (
        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 24, flexWrap: 'wrap' }}>
          <div>
            <span className="serif" style={{ fontSize: 22, fontStyle: 'italic', color: 'var(--ink-2)' }}>
              "{data.body[0].replace(/\{[a-z]+:([^}]+)\}/g, '$1').slice(0, 110)}…"
            </span>
          </div>
          <button className="btn btn-ghost btn-sm" onClick={() => onMarkRead(false)}>
        ...
        <>
          ...
          {/* Body paragraphs — larger, more breathing */}
          <div style={{ maxWidth: 920, margin: '40px 0 0' }}>
            {data.body.map((line, i) => (
              <p key={i} className="briefing-body" style={{
                margin: i === 0 ? '0 0 22px 0' : '0 0 22px 0',
          ...
          {/* Inline metrics — pushed to bottom of fold */}
          <div style={{ marginTop: 'auto', paddingTop: 48 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 28, flexWrap: 'wrap' }}>
              ...
              <button className="btn btn-ghost press" onClick={() => onMarkRead(true)}>
...
function PageHead({ today, time, onCloseClick, dayClosed }) {
  return (
    <div style={{ padding: '36px 0 12px', display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 32, flexWrap: 'wrap' }}>
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
        ...
        {onCloseClick && (
          <button className="btn press" onClick={onCloseClick} style={dayClosed ? { borderColor: 'var(--ok)', color: 'var(--ok)' } : {}}>
            <span className="material-symbols-outlined" style={{ fontSize: 16 }}>
              {dayClosed ? 'check_circle' : 'nightlight'}
            </span>
            {dayClosed ? 'Día cerrado' : 'Cerrar el día'}
          </button>
        )}
      </div>
    </div>
...
// ============================================================================
// CerrarElDia — closing ritual at the end of the day
// ============================================================================

function CerrarElDia({ data, isClosed, onClose, onReopen }) {
  const totalDone = data.done.length;
  const totalPending = data.pendingFlat.length;

  if (isClosed) {
    return (
      <section data-screen-label="07 Cerrado" style={{
        padding: '80px 0 60px',
        borderTop: '1px solid var(--rule-2)',
        textAlign: 'center',
      }}>
        <div style={{ maxWidth: 560, margin: '0 auto' }}>
          ...
          <h2 className="serif" style={{
            ...
            <em>Buen trabajo, Luis.</em>
          </h2>
          <p className="briefing-body" style={{ margin: '0 auto', maxWidth: 480, fontSize: 19 }}>
            {data.hermesTomorrow}
          </p>
          <div style={{ marginTop: 32, display: 'flex', justifyContent: 'center', gap: 10 }}>
            <button className="btn btn-ghost press" onClick={onReopen}>
              <span className="material-symbols-outlined" style={{ fontSize: 16 }}>undo</span>
              Reabrir el día
            </button>
          </div>
          <div style={{ marginTop: 56, color: 'var(--ink-4)' }} className="mono meta">
            — Hermes
          </div>
        </div>
      </section>
  ...
  return (
    <section data-screen-label="07 Cierre" style={{
      padding: '60px 0 48px',
      borderTop: '1px solid var(--rule-2)',
      background: 'linear-gradient(180deg, transparent, var(--paper-2) 80%)',
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 32, gap: 24, flexWrap: 'wrap' }}>
        <div>
          <span className="eyebrow" style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
            <span className="material-symbols-outlined" style={{ fontSize: 14 }}>nightlight</span>
            Cerrar el día
          </span>
          <h2 className="serif" style={{
            margin: '8px 0 0',
            ...
            Hoy avanzaste con <em>{totalDone}</em> cosas.
          </h2>
        </div>
        <span className="mono meta">Resumen generado por Hermes</span>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 56, alignItems: 'flex-start' }}>
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
            <span className="label">Hecho hoy</span>
            <span className="mono meta">· {totalDone} acciones</span>
          </div>
          <div>
            {data.done.map((d, i) => (
              <div key={i} style={{
                ...
                <span className="material-symbols-outlined" style={{ fontSize: 16, color: 'var(--ok)', paddingTop: 2 }}>
                  check
                </span>
                <div>
                  <div style={{ fontSize: 14, color: 'var(--ink)', fontWeight: 500 }}>{d.what}</div>
                  {d.detail && <div className="meta">{d.detail}</div>}
                </div>
              </div>
            ))}
          </div>
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
            <span className="label">Queda abierto</span>
            <span className="mono meta">· {totalPending} pendiente{totalPending === 1 ? '' : 's'}</span>
          </div>
          <div>
            {data.pendingFlat.map((p, i) => (
              <div key={i} style={{
                display: 'grid', gridTemplateColumns: '24px 1fr',
                gap: 12, padding: '10px 0',
                borderTop: '1px solid var(--rule-2)',
              }}>
                <span className="material-symbols-outlined" style={{ fontSize: 16, color: 'var(--ink-4)', paddingTop: 2 }}>
                  schedule
                </span>
                <div>
                  <div style={{ fontSize: 14, color: 'var(--ink)' }}>{p.what}</div>
                  <div className="meta">{p.when}</div>
                </div>
              </div>
            ))}
            <div style={{ padding: '10px 0', borderTop: '1px solid var(--rule-2)' }}>
              <div className="meta" style={{ fontStyle: 'italic' }}>
                Hermes los retoma mañana en el briefing.
              </div>
            </div>
          </div>
        </div>
      ...
      <div style={{
        ...
        <div className="briefing-body" style={{ fontSize: 18, maxWidth: 560 }}>
          <em>{data.hermesTomorrow}</em>
        </div>
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          <button className="btn btn-primary press" onClick={onClose}>
            <span className="material-symbols-outlined" style={{ fontSize: 16 }}>nightlight</span>
            Cerrar el día
          </button>
        </div>
      </div>
...
Object.assign(window, {
  Briefing, Priorities, Agenda, Pulse, Drafts, ActivityLog, TopBar, PageHead, Sparkline, CerrarElDia
});
