...
// Orientation slug — strips accent + lowercases so it can be a CSS data attr
const orientSlug = (o) => {
  if (!o) return '';
  return o.toString().normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase();
};
// Quick CSS-var lookup for inline use (sparklines, progress bars)
const orientColor = (o) => `var(--orient-${orientSlug(o)})`;

// ============================================================================
// Canvas header — title, status, action bar (shared across states)
// ============================================================================

// Status pipeline — full-width tool-style timeline. Each step is clickable
// (currently visual only) and the active step is filled-ink for hard
// "you-are-here" signal. Designed to replace the editorial step row.
function StatusPipeline({ currentStep, currentState }) {
  // currentStep is 1..5 (Crear..Activa) or 6 (Archivada).
  // For archivada, we show the last step (Activa) as 'done' AND surface an
  // "Archivada" badge separately in the header.
  const idx = currentStep > 5 ? 5 : currentStep;
  return (
    <div className="pipeline">
      {STEPS.map((s, i) => {
        ...
        const cls = `pipeline-step ${done ? 'done' : ''} ${active ? 'active' : ''}`;
        return (
...
// Kept for any place that still references it (legacy).
function StepIndicator({ currentStep }) {
  return <StatusPipeline currentStep={currentStep} />;
}

function CanvasHeader({ c, primaryAction, secondaryActions = [] }) {
  const state = STATES[c.state];
  return (
    <header style={{ padding: '24px 48px 22px', borderBottom: '1px solid var(--rule-2)' }}>
      {/* Row 1 — context strip: chip + institution + ID/time */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14, flexWrap: 'wrap' }}>
        <span className="chip-status" data-state={c.state}>
          <span className={`dot dot-state-${c.state}`}></span>
          {state.label}
        </span>
        {c.institucion && (
          <span className="meta" style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
            <span className="material-symbols-outlined" style={{ fontSize: 14, color: 'var(--ink-4)' }}>apartment</span>
            {c.institucion.nombre}
          </span>
        )}
        <span className="mono meta" style={{ marginLeft: 'auto', fontSize: 11 }}>
          ID&nbsp;{c.id.toUpperCase()}
          {c.updatedAt && <span style={{ color: 'var(--ink-4)' }}> · editada {c.updatedAt.toLowerCase()}</span>}
        </span>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 24, marginBottom: 18 }}>
        <div style={{ minWidth: 0, flex: 1 }}>
          <h1 className="serif" style={{
            margin: 0, fontSize: 30, lineHeight: 1.15, fontWeight: 700,
            letterSpacing: '-0.025em', color: 'var(--ink)',
          }}>
            {c.nombre || <span style={{ color: 'var(--ink-4)', fontWeight: 500 }}>Convocatoria sin nombre</span>}
          </h1>
          {c.orientaciones && c.orientaciones.length > 0 && (
            <div style={{ display: 'flex', gap: 8, marginTop: 14, flexWrap: 'wrap' }}>
              {c.orientaciones.map(o => (
                <span key={o} className="chip-orient" data-orient={orientSlug(o)} style={{ cursor: 'default' }}>{o}</span>
              ))}
            </div>
          )}
        </div>

        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexShrink: 0, marginTop: 4, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
      ...
      {/* Row 3 — pipeline */}
      <StatusPipeline currentStep={state.step > 5 ? 6 : state.step} currentState={c.state} />
    </header>
  );
}
            ...
            <BorradorSection number="01" title="La convocatoria" subtitle="Institución, nombre y orientación" missing={sectionMissing.s1.length}>
              ...
              <FormRow label="Orientaciones">
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  {ORIENTACIONES.map(o => {
                    const on = form.orientaciones.includes(o);
                    return (
                      <button
                        key={o}
...
function StudentCardPreview({ form }) {
  ...
  const cardOrientColor = first !== '—' ? orientColor(first) : 'var(--ink-4)';
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
        <span className="chip-orient" data-orient={orientSlug(first)} style={{ cursor: 'default', fontSize: 11 }}>{first}</span>
        ...
        <div style={{
          ...
          <div style={{ paddingLeft: 24 }}>
            ...
            <div style={{ marginTop: 8, display: 'flex', flexDirection: 'column', gap: 8 }}>
              <MixRow data={c.mix.orientaciones} total={c.inscriptos} label="Orientación" colorBy="orientacion" />
...
function MixRow({ data, total, label, colorBy }) {
  const items = Object.entries(data).filter(([_, v]) => v > 0);
  const colorFor = (k, i) => {
    if (colorBy === 'orientacion') return orientColor(k);
    // Default: monochrome ramp
    return ['var(--ink)', 'var(--ink-3)', 'var(--ink-4)', 'var(--rule-3)'][i % 4];
  };
  return (
