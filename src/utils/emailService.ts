...
const getBlockConfig = (title: string) => {
    ...
    if (lower.includes('puntualidad') || lower.includes('asistencia')) return { titleColor: '#1e40af', bg: '#eff6ff', border: '#bfdbfe' };
    if (lower.includes('ética') || lower.includes('confidencialidad')) return { titleColor: '#047857', bg: '#ecfdf5', border: '#a7f3d0' };
    if (lower.includes('rol') || lower.includes('activo')) return { titleColor: '#7e22ce', bg: '#faf5ff', border: '#e9d5ff' };
    if (lower.includes('documentación')) return { titleColor: '#be123c', bg: '#fff1f2', border: '#fecdd3' };
...
export const generateHtmlTemplate = (textBody: string, title: string = "Comunicación Institucional"): string => {
    const cleanText = stripGreeting(textBody).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (!line) { contentHtml += '<div style="height: 12px;">&nbsp;</div>'; continue; }
    ...
    const headerStyle = "background: linear-gradient(135deg, #00B2A9 0%, #1e40af 100%); padding: 32px 40px;";
