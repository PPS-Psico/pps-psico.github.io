...
const NavLink: React.FC<{
  ...
  <button
    onClick={onClick}
    className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-semibold transition duration-200 ${
      isActive
...
const StudentLink: React.FC<{
  ...
  <div className="relative group">
    <button
      onClick={onClick}
      className={`w-full flex items-center gap-3 pl-4 pr-9 py-2.5 rounded-lg text-sm font-medium transition duration-200 ${
        isActive
...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <aside
      className={`flex-shrink-0 glass-panel p-4 rounded-2xl transition duration-300 ${className}`}
    >
