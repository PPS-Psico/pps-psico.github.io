  ...
  (
    ...
    >
      <div className="relative flex items-center h-6">
        ...
        <div
          className={`
            h-5 w-5 rounded border-2 transition duration-200 ease-out flex items-center justify-center
      ...
      {(label || sublabel) && (
        <div className="flex flex-col select-none">
          {label && (
            <span
              className={`text-sm font-medium transition-colors ${error ? "text-red-600 dark:text-red-400" : "text-slate-700 dark:text-slate-200 group-hover:text-amber-900 dark:group-hover:text-blue-100"} ${labelClassName}`}
            >
              {label}
            </span>
          )}
