...
const ActionItem: React.FC<ActionItemProps> = ({
  ...
  return (
    ...
    >
      ...
      <div
        className={`relative z-10 p-3 rounded-xl ${colorClass} bg-opacity-10 dark:bg-opacity-20`}
      >
      ...
      <div className="relative z-10 flex-grow">
        <div className="flex justify-between items-start">
          <h4 className="font-bold text-slate-800 dark:text-slate-100 group-hover:text-slate-900 dark:group-hover:text-white transition-colors">
            {title}
          </h4>
          {count > 0 && (
            <span
              className={`text-xs font-bold px-2 py-0.5 rounded-full ${colorClass} bg-opacity-10`}
            >
              {count}
            </span>
          )}
