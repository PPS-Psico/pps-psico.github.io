...
const DataIntegrityTool: React.FC = () => {
  ...
  const fixDatabaseStates = async () => {
    ...
    try {
      ...
      setToast({
        m: `¡Éxito! Se corrigieron ${zombies.length} estados "Nuevo" a "Inactivo".`,
        t: "success",
      });
  ...
  const sanitizeNames = async () => {
    ...
    try {
      ...
      for (const inst of dirtyInstitutions) {
        ...
        if (newName && newName !== oldName) {
          await supabase
            ...
            .update({ [FIELD_NOMBRE_INSTITUCIONES]: newName })
            .eq("id", inst.id);
          fixedCount++;
        }
      }
      ...
      const lanzamientos = await db.lanzamientos.getAll({
      ...
      for (const lanz of dirtyLaunches) {
        ...
        if (newName && newName !== oldName) {
          await supabase
            ...
            .update({ [FIELD_NOMBRE_PPS_LANZAMIENTOS]: newName })
            .eq("id", lanz.id);
          fixedCount++;
        }
      }
      ...
      const practicas = await db.practicas.getAll({
        fields: [FIELD_NOMBRE_INSTITUCION_LOOKUP_PRACTICAS],
      });
      ...
      if (fixedCount > 0) {
        setToast({
          m: `Se limpiaron ${fixedCount} registros con corchetes/comillas en Instituciones, Lanzamientos y Prácticas.`,
          t: "success",
        });
        queryClient.invalidateQueries();
      } else {
        setToast({
          m: "No se encontraron registros sucios. La base de datos está limpia.",
          t: "success",
        });
      }
    } catch (e: any) {
        ...
        <div className="space-y-3">
          <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-xl border border-blue-100 dark:border-blue-800">
            ...
            <p className="text-xs text-blue-800 dark:text-blue-300 leading-relaxed">
              Analiza si un alumno tiene datos de contacto (Mail/DNI) pero figura como "Nuevo".
              Actualiza a "Inactivo" para que pueda operar.
            </p>
          </div>
          <Button
        ...
        <div className="space-y-3">
          <div className="bg-amber-50 dark:bg-amber-900/20 p-4 rounded-xl border border-amber-100 dark:border-amber-800">
            <h4 className="text-sm font-bold text-amber-900 dark:text-amber-100 mb-1">
              Sanitizador de Texto (Migración)
            </h4>
            <p className="text-xs text-amber-800 dark:text-amber-300 leading-relaxed">
              Escanea <strong>Instituciones, Lanzamientos y Prácticas</strong> y elimina caracteres
              basura como <code>["..."]</code>, <code>&#123;...&#125;</code> o comillas extra que
              quedaron de la importación.
            </p>
          </div>
          <Button
