  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <span
          className={`material-icons !text-sm transition-opacity ${isActive ? "opacity-100 text-blue-600 dark:text-blue-400" : "opacity-30 group-hover:opacity-70"}`}
        >
          {icon}
        </span>
      </div>
...
const DatabaseEditor: React.FC<DatabaseEditorProps> = ({ isTestingMode = false }) => {
  ...
  const [editingRecord, setEditingRecord] = useState<AppRecord<any> | { isCreating: true } | null>(
    null
  );
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const { records, total, error } = await db[activeTable].getPage(currentPage, itemsPerPage, {
      ...
      if (activeTable === "practicas" && records.length > 0) {
        ...
        const enrichedRecords = records.map((p) => {
          ...
          const lanzamientoId = Array.isArray(rawLanzamientoId)
            ? rawLanzamientoId[0]
            : rawLanzamientoId;
          ...
          const lanzamientoName =
            lanzamientosMap.get(lanzamientoId as string)?.[FIELD_NOMBRE_PPS_LANZAMIENTOS] ||
  ...
  const tableTabs = Object.entries(EDITABLE_TABLES).map(([key, { label, icon }]) => ({
    id: key,
    label,
    icon,
  }));
  ...
  const renderCellValue = (record: AppRecord<any>, fieldConfig: any) => {
    ...
    if (fieldConfig.type === "checkbox") {
      return value ? (
        <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-200">
          Sí
        </span>
      ) : (
    ...
    if (
          ...
          <div className="flex flex-wrap gap-1">
            {list.map((o, i) => {
              ...
              return (
                <span
                  key={i}
                  className={`${visuals.tag} text-[10px] px-1.5 py-0.5 rounded whitespace-nowrap shadow-none border-0`}
                >
                  {o}
                </span>
              );
      ...
      return (
        <span className={`${visuals.tag} whitespace-nowrap shadow-none border-0`}>
          {String(value)}
        </span>
      );
    }
    ...
    <Card title="Editor de Base de Datos" icon="storage">
      ...
      {contextMenu && (
        <ContextMenu
          ...
          onDuplicate={() => {
            duplicateMutation.mutate(contextMenu.record);
            setContextMenu(null);
          }}
          onDelete={() => {
        ...
        <div className="flex flex-col md:flex-row justify-between items-start gap-4 mb-6">
          <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto flex-wrap">
            ...
            <div className="relative w-full md:w-72 group">
              ...
              />
              <span className="absolute left-3 top-1/2 -translate-y-1/2 material-icons text-slate-400 !text-lg pointer-events-none">
                search
              </span>
              {searchTerm && (
          ...
          <div className="flex items-center gap-2 w-full md:w-auto">
            ...
            <button
              ...
              className="w-full md:w-auto bg-blue-600 text-white font-bold py-2.5 px-5 rounded-lg text-sm flex items-center justify-center gap-2 hover:bg-blue-700 transition-all shrink-0"
            >
                  ...
                  <tr>
                    {activeTableConfig.displayFields.map((key) => {
                      ...
                      const label = fieldConfig
                        ? fieldConfig.label
                        : key.startsWith("__")
                          ? key.substring(2).replace(/([A-Z])/g, " $1")
                          : key;
                      const className =
                        key === "__lanzamientoName" || key === FIELD_NOMBRE_ESTUDIANTES
                          ? "min-w-[250px]"
                          : "";
                      return (
                        <SortableHeader
                          ...
                          className={`transition-colors cursor-pointer ${
                            isSelected
                              ...
                              : idx % 2 === 0
                                ...
                                : "bg-slate-50/30 dark:bg-slate-800/50 hover:bg-slate-100 dark:hover:bg-slate-700/70"
                          }`}
                        >
                          {activeTableConfig.displayFields.map((key) => {
                            const fieldConfig = activeTableConfig.fieldConfig.find(
                              (f) => f.key === key
                            ) || { key };
                            return (
                              <td
                                key={key}
                                className="px-6 py-3 text-slate-700 dark:text-slate-300 align-middle"
                              >
