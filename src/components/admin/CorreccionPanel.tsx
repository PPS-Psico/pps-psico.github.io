...
import { fetchCorrectionPanelData } from "../../services";
...
const CorreccionPanel: React.FC<CorreccionPanelProps> = ({ isTestingMode = false }) => {
  ...
  const ppsGroups = useMemo<InformeCorreccionPPS[]>(
    () => Array.from(ppsGroupsMap?.values() || []),
    [ppsGroupsMap]
  );
  ...
  if (error)
    return (
      <EmptyState
        icon="error"
        title="Error"
        message="No se pudieron cargar los datos de corrección."
      />
    );
        ...
        <div className="grid grid-cols-1 gap-6">
          {ppsGroups.length > 0 ? (
              ...
              <InformeCorreccionCard
                ...
                onBatchUpdate={async (nota) => {
                  ...
                  try {
                    ...
                    setToastInfo({
                      message: "Funcionalidad de actualización por lote próximamente disponible.",
                      type: "warning",
                    });
                  } finally {
            ...
            <EmptyState
              ...
              message="No hay informes para corregir actualmente."
            />
          )}
        </div>
      ) : (
