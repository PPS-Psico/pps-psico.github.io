...
const MOCK_STUDENTS_FOR_SEARCH: any[] = [
  {
    id: "recTest1",
    createdTime: "",
    [FIELD_LEGAJO_ESTUDIANTES]: "T0001",
    [FIELD_NOMBRE_ESTUDIANTES]: "Tester Alfa",
  },
  {
    id: "recTest2",
    createdTime: "",
    [FIELD_LEGAJO_ESTUDIANTES]: "T0002",
    [FIELD_NOMBRE_ESTUDIANTES]: "Beta Tester",
  },
  {
    id: "recTest3",
    createdTime: "",
    [FIELD_LEGAJO_ESTUDIANTES]: "T0003",
    [FIELD_NOMBRE_ESTUDIANTES]: "Gama Tester",
  },
];
...
const AdminSearch: React.FC<AdminSearchProps> = ({
    ...
    async (term: string) => {
      ...
      if (isTestingMode) {
        setTimeout(() => {
          ...
          const filtered = MOCK_STUDENTS_FOR_SEARCH.filter(
            (s) =>
  ...
  return (
    <div
      ...
      className={`relative w-full h-full ${showDropdown ? "z-[100]" : "z-auto"}`}
    >
      ...
      {showDropdown && (
        <div
          className="absolute top-full left-0 z-[100] mt-2 w-full bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-200/80 dark:border-slate-700 overflow-hidden max-h-80 overflow-y-auto animate-fade-in-up"
            ...
            <ul>
              {results.map((student) => (
                <li key={student.id}>
                  ...
                  >
                    <span className="font-semibold text-slate-800 dark:text-slate-100 text-sm group-hover:text-blue-700 dark:group-hover:text-blue-200">
                      {student[FIELD_NOMBRE_ESTUDIANTES]}
                    </span>
                    <span className="text-xs font-mono text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-700/50 px-2 py-0.5 rounded border border-slate-200 dark:border-slate-700/50 group-hover:border-blue-200 dark:group-hover:border-blue-800">
