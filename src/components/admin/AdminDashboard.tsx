...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ isTestingMode = false }) => {
  ...
  const overdueReminderIds = new Set(overdueReminders.map((reminder) => reminder.id));
  const agendaReminders = [
    ...overdueReminders.map((reminder) => ({ reminder, label: "Vencido", tone: "rose" as const })),
    ...todayReminders
      .filter((reminder) => !overdueReminderIds.has(reminder.id))
      .map((reminder) => ({ reminder, label: "Hoy", tone: "amber" as const })),
  ].slice(0, 4);
  const laterThisWeekCount = Math.max(counts.week - counts.today, 0);
  const shouldShowAgenda =
    isRemindersLoading || counts.overdue > 0 || counts.today > 0 || laterThisWeekCount > 0;

  const completeDashboardReminder = async (reminderId: string) => {
    const completed = await completeReminder(reminderId);
    setToastInfo({
      message: completed ? "Recordatorio completado" : "No se pudo completar el recordatorio",
  ...
  return (
    <div className="space-y-12 animate-fade-in-up pb-10">
      ...
      {/* --- AGENDA: RECORDATORIOS Y SEGUIMIENTOS --- */}
      {shouldShowAgenda && (
        <section className="mx-4 md:mx-0 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm overflow-hidden">
          <div className="flex flex-col gap-4 border-b border-slate-100 dark:border-slate-800 p-5 md:flex-row md:items-center md:justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400">
                  Recordatorios vencidos, tareas de hoy y seguimientos de esta semana.
                </p>
              </div>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              {counts.overdue > 0 && (
                <span className="rounded-full bg-rose-50 px-3 py-1 text-xs font-bold text-rose-700 dark:bg-rose-900/20 dark:text-rose-300">
                  {counts.overdue} vencidos
                </span>
              )}
              {counts.today > 0 && (
                <span className="rounded-full bg-amber-50 px-3 py-1 text-xs font-bold text-amber-700 dark:bg-amber-900/20 dark:text-amber-300">
                  {counts.today} hoy
                </span>
              )}
              {laterThisWeekCount > 0 && (
                <span className="rounded-full bg-blue-50 px-3 py-1 text-xs font-bold text-blue-700 dark:bg-blue-900/20 dark:text-blue-300">
                  {laterThisWeekCount} esta semana
                </span>
              )}
              <button
                onClick={() => navigate("/admin/gestion?view=agenda")}
                className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-2 text-xs font-bold text-slate-700 transition-colors hover:bg-slate-50 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800"
              >
                <span className="material-icons !text-base">notifications_active</span>
                Ver todos
              </button>
            </div>
          </div>
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-800">
            {isRemindersLoading && agendaReminders.length === 0 ? (
              <div className="flex items-center gap-3 p-5 text-sm text-slate-500 dark:text-slate-400">
                <span className="material-icons animate-spin">progress_activity</span>
                Cargando agenda...
              </div>
            ) : agendaReminders.length > 0 ? (
              agendaReminders.map(({ reminder, label, tone }) => (
                <div
                  key={reminder.id}
                ...
                >
                  <div className="flex min-w-0 items-start gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      {(reminder.pps_name || reminder.description) && (
                  ...
                  <div className="flex shrink-0 flex-wrap items-center gap-2">
                    {reminder.institution_phone && (
                      <a
                        href={`https://wa.me/${reminder.institution_phone.replace(/[^0-9]/g, "")}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-2 rounded-xl bg-emerald-50 px-3 py-2 text-xs font-bold text-emerald-700 transition-colors hover:bg-emerald-100 dark:bg-emerald-900/20 dark:text-emerald-300 dark:hover:bg-emerald-900/30"
                      >
                    ...
                    <button
                      onClick={() => void completeDashboardReminder(reminder.id)}
                      className="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-3 py-2 text-xs font-bold text-white transition-colors hover:bg-slate-700 dark:bg-white dark:text-slate-900 dark:hover:bg-slate-200"
                    >
              ...
              >
                <div>
                  <p className="text-sm font-bold text-slate-900 dark:text-white">
                    No hay recordatorios vencidos ni para hoy.
                  </p>
                  <p className="text-sm text-slate-500 dark:text-slate-400">
                    Tenes {laterThisWeekCount} seguimiento(s) programado(s) para esta semana.
                  </p>
                </div>
