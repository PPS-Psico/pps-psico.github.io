...
const ConvocatoriaManager: React.FC<ConvocatoriaManagerProps> = ({ forcedOrientations, isTestingMode = false }) => {
    ...
    return (
        <div className="animate-fade-in-up space-y-8">
            ...
            {/* SECCIÓN 1: ACTIVAS Y POR FINALIZAR (PRIORIDAD ALTA) */}
            {filteredData.activasYPorFinalizar.length > 0 && filterType === 'all' && (
                <CollapsibleSection
            ...
            {/* SECCIÓN 2: FINALIZADAS PARA REACTIVAR (ACCIÓN REQUERIDA) */}
            {filteredData.finalizadasParaReactivar.length > 0 && filterType === 'all' && (
                <CollapsibleSection
                    title="Finalizadas - Requieren Gestión"
                ...
                >
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mt-4 items-start">
                        {filteredData.finalizadasParaReactivar.map((pps: any) => (
                            <GestionCard
                                key={pps.id}
                                pps={pps}
                                onSave={handleSave}
                                ...
                                institution={institutionsMap.get(normalizeStringForComparison(pps[FIELD_NOMBRE_PPS_LANZAMIENTOS] || ''))}
                                onSavePhone={handleUpdateInstitutionPhone}
                                daysLeft={-(pps.daysSinceEnd || 0)} // Negative means finished
                            />
                        ))}
                    </div>
                </CollapsibleSection>
            ...
            {/* SECCIÓN 3: RELANZAMIENTOS CONFIRMADOS */}
            {filteredData.relanzamientosConfirmados.length > 0 && filterType === 'all' && (
                <CollapsibleSection
                    title="Relanzamientos Confirmados (Planificados)"
            ...
            {/* SECCIÓN 4: INDEFINIDAS (MANUALES) */}
            {filteredData.activasIndefinidas.length > 0 && filterType === 'all' && (
                <CollapsibleSection
            ...
            {filteredData.activasYPorFinalizar.length === 0 && filteredData.finalizadasParaReactivar.length === 0 && filteredData.relanzamientosConfirmados.length === 0 && (
                <EmptyState
                    icon="assignment_turned_in" // Changed icon
                    title="Todo al día"
                    message="No hay instituciones requiriendo atención inmediata."
                />
            )}
        </div>
    );
