...
const ActiveInstitutionsReport: React.FC<{ isTestingMode?: boolean }> = ({
  isTestingMode = false,
}) => {
  ...
  const reportData = useMemo((): ReportData[] => {
    ...
    launchesThisYear.forEach((launch) => {
      ...
      if (orientacion && !entry.orientaciones.includes(orientacion)) {
        entry.orientaciones = entry.orientaciones
          ? `${entry.orientaciones}, ${orientacion}`
          : orientacion;
      }
    });
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const titleRow = worksheet.addRow([
        `Reporte de Instituciones Activas ${new Date().getFullYear()}`,
      ]);
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
        <div>
          <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100">
            Reporte de Instituciones Activas
          </h2>
          <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">
            Instituciones con actividad durante {new Date().getFullYear()}.
          </p>
        </div>
        <button
