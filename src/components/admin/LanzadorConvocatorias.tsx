            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-slate-200 dark:border-slate-700" onClick={e => e.stopPropagation()}>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    ...
                    <div className="pt-2 border-t border-slate-100 dark:border-slate-700 mt-2">
                        <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2 ml-1">Logo Institucional (Opcional)</label>
                        <div className="flex items-center gap-4">
                            ...
                            {newData.logoFile && <span className="text-sm text-emerald-600 dark:text-emerald-400 font-medium truncate max-w-[150px]">{newData.logoFile.name}</span>}
                        </div>
...
const LAUNCH_TABLE_CONFIG = {
    ...
    fieldConfig: [
        ...
        { key: FIELD_ESTADO_CONVOCATORIA_LANZAMIENTOS, label: 'Estado', type: 'select' as const, options: ['Abierta', 'Cerrado', 'Oculto'] },
...
🔗 *Inscripción en Mi Panel.*`;
    ...
    const handleLoadLastData = useCallback(() => {
        ...
        setFormData(prev => ({
            ...
            requisitoObligatorio: (lastLanzamiento[FIELD_REQUISITO_OBLIGATORIO_LANZAMIENTOS] as string) || '',
            fechaInicioInscripcion: '',
            fechaFinInscripcion: '',
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        const fechaRealInicio = formData.programarLanzamiento && formData.fechaPublicacion
        ...
        if (!formData.nombrePPS || !fechaRealInicio || !formData.orientacion || !hasHours) {
            setToastInfo({ message: 'Por favor, complete los campos requeridos (Nombre, Fecha, Orientación, Horas).', type: 'error' });
            return;
        }
        ...
        const finalPayload = {
            ...
            [FIELD_ESTADO_CONVOCATORIA_LANZAMIENTOS]: formData.programarLanzamiento ? 'Programada' : formData.estadoConvocatoria,
            ...
            [FIELD_FECHA_PUBLICACION_LANZAMIENTOS]: formData.programarLanzamiento ? formData.fechaPublicacion : null,
    ...
    const renderLaunchItem = useCallback((launch: LanzamientoPPS) => {
        ...
        return (
            <div key={launch.id} className={`bg-white dark:bg-slate-800/50 p-4 rounded-xl border transition-all hover:shadow-md ${isAbierta ? 'border-emerald-300 dark:border-emerald-800 ring-1 ring-emerald-100 dark:ring-emerald-900/30' : (isProgramada ? 'border-indigo-300 dark:border-indigo-800 ring-1 ring-indigo-50 dark:ring-indigo-900/20' : (isOculta ? 'border-slate-200 dark:border-slate-700 opacity-75' : 'border-slate-200 dark:border-slate-700'))} flex flex-col md:flex-row justify-between items-start md:items-center gap-4`}>
                <div className="flex-1">
                    <div className="flex items-center flex-wrap gap-2 mb-1">
                        <h4 className="font-bold text-slate-800 dark:text-slate-100">{launch[FIELD_NOMBRE_PPS_LANZAMIENTOS]}</h4>
                        <div className="flex items-center gap-1.5">
                            <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded-full border ${isAbierta ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : (isProgramada ? 'bg-indigo-50 text-indigo-700 border-indigo-200' : (isOculta ? 'bg-slate-100 text-slate-500 border-slate-200' : 'bg-slate-100 text-slate-500 border-slate-200'))}`}>
                    ...
                    <p className="text-sm text-slate-500 dark:text-slate-400">
                        {isProgramada ? `Publicación: ${formatDate(pubDate)}` : `Inicio: ${formatDate(launch[FIELD_FECHA_INICIO_LANZAMIENTOS] || '')}`} &bull; Orientación: {launch[FIELD_ORIENTACION_LANZAMIENTOS]}
                    </p>
                </div>
                <div className="flex items-center gap-2">
                    ...
                    {isOculta ? (
                        <button onClick={() => handleStatusAction(launch.id, 'cerrar')} className="p-2 text-slate-500 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Hacer Visible (Cerrada)">
                            <span className="material-icons !text-xl">visibility</span>
                        </button>
                    ) : isAbierta || isProgramada ? (
                        <button onClick={() => handleStatusAction(launch.id, 'cerrar')} className="p-2 text-slate-500 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Cerrar/Cancelar">
                            <span className="material-icons !text-xl">{isProgramada ? 'event_busy' : 'lock'}</span>
                        </button>
                    ) : (
                        <button onClick={() => handleStatusAction(launch.id, 'abrir')} className="p-2 text-slate-500 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors" title="Reabrir Convocatoria">
                            <span className="material-icons !text-xl">lock_open</span>
                        </button>
                    )}
                    {!isOculta && (
                        <button onClick={() => handleStatusAction(launch.id, 'ocultar')} className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg transition-colors" title="Ocultar">
                ...
                <form onSubmit={handleSubmit} className="mt-8 space-y-8 animate-fade-in">
                            ...
                            <div className="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm relative">
                                <div className="flex flex-col md:flex-row gap-4 items-end">
                                    <div className="relative flex-grow w-full">
                                        <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Buscar Institución</label>
                        ...
                        <div className="pl-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 bg-slate-50 dark:bg-slate-800/50 p-6 rounded-xl border border-slate-200 dark:border-slate-700">
                                <InputWrapper label="Nombre PPS (Visible)" icon="badge">
                                    <Input value={formData.nombrePPS as string} onChange={handleChange} name="nombrePPS" placeholder="Nombre visible de la convocatoria" />
                                </InputWrapper>
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        <div className="flex items-center">
                                            <Checkbox
                                                ...
                                                onChange={(e) => setFormData(prev => ({ ...prev, direccion: e.target.checked ? 'Modalidad Virtual' : '' }))}
                                ...
                                <InputWrapper label="Fecha Inicio" icon="event_available">
                                    <Input type="date" value={formData.fechaInicio as string} onChange={handleChange} name="fechaInicio" />
                                </InputWrapper>

                                <InputWrapper label="Fecha Fin (Práctica)" icon="event_busy">
                                    <Input type="date" value={formData.fechaFin as string} onChange={handleChange} name="fechaFin" />
                                </InputWrapper>
                                ...
                                <InputWrapper label="Inscripción Desde" icon="app_registration">
                                    <Input type="date" value={formData.fechaInicioInscripcion as string} onChange={handleChange} name="fechaInicioInscripcion" />
                                </InputWrapper>

                                <InputWrapper label="Inscripción Hasta" icon="access_time_filled">
                                    <Input type="date" value={formData.fechaFinInscripcion as string} onChange={handleChange} name="fechaFinInscripcion" />
                                </InputWrapper>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <InputWrapper label="Horas" icon="schedule">
                                        <Input type="number" value={formData.horasAcreditadas as number} onChange={handleChange} name="horasAcreditadas" />
                                    </InputWrapper>
                                    <InputWrapper label="Cupos" icon="group">
                                        <Input type="number" value={formData.cuposDisponibles as number} onChange={handleChange} name="cuposDisponibles" />
                                    </InputWrapper>
                                </div>
                                ...
                                <div className="flex flex-col gap-3 pt-2 md:col-span-2 lg:col-span-3">
                                    <label className="text-xs font-bold text-slate-500 uppercase tracking-wider flex items-center gap-2">
                                        <span className="material-icons text-slate-300 !text-sm">verified_user</span>Requisitos
                                    </label>
                                    <div className="flex flex-wrap items-center gap-6">
                                        <Checkbox
                                            label="Cert. Trabajo"
                                            checked={formData.reqCertificadoTrabajo}
                                            onChange={(e) => setFormData(prev => ({ ...prev, reqCertificadoTrabajo: e.target.checked }))}
                                        />
                                        <Checkbox
                                            label="Solicitar CV"
                                            checked={formData.reqCv}
                                            onChange={(e) => setFormData(prev => ({ ...prev, reqCv: e.target.checked }))}
                                        />
                                    </div>
                                </div>
                                ...
                                <div className="md:col-span-2 lg:col-span-3 flex flex-col md:flex-row gap-4 p-4 bg-blue-50 dark:bg-blue-900/10 rounded-xl border border-blue-100 dark:border-blue-800/30 mt-2">
                                    <div className="flex-1">
                                        <Checkbox
                                            ...
                                            onChange={(e) => setFormData(prev => ({ ...prev, programarLanzamiento: e.target.checked }))}
                                        />
                                        <p className="text-[10px] text-slate-500 mt-1 pl-7">
                                            Si activas esto, la convocatoria se publicará automáticamente en la fecha y hora seleccionada.
                                        </p>
                                    </div>
                        ...
                        <div className="pl-6">
                            <div className="flex items-center justify-between mb-4">
                                <h3 className="text-lg font-bold text-slate-800 dark:text-white flex items-center gap-2">
                                    <span className="flex items-center justify-center w-8 h-8 rounded-full bg-purple-100 dark:bg-purple-900 text-purple-600 dark:text-purple-300 text-sm font-bold shadow-sm border border-purple-200 dark:border-purple-800">3</span>
                            ...
                            <div className="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-xl border border-slate-200 dark:border-slate-700 space-y-6">
                                ...
                                <div>
                                    <div className="flex items-center justify-between mb-2 ml-1">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <span className="text-[10px] text-slate-400 font-bold uppercase">Título:</span>
                                            <input
                                                type="text"
                                    ...
                                    <div className="space-y-3">
                                        {actividades.map((act, idx) => (
                                            <div key={idx} className="flex gap-2">
                                                ...
                                                />
                                                <button type="button" onClick={() => removeActivity(idx)} className="p-2 hover:bg-rose-100 hover:text-rose-600 text-slate-400 rounded-lg transition-colors">
                                                    <span className="material-icons !text-lg">delete</span>
                                                </button>
                                            </div>
                                        ))}
                                        <button type="button" onClick={addActivity} className="text-sm font-bold text-purple-600 hover:text-purple-700 flex items-center gap-1 mt-2 px-2">
                    ...
                    <div className="relative group z-20">
                        ...
                        <div className="pl-6">
                            <div className="flex items-center justify-between mb-4">
                                <h3 className="text-lg font-bold text-slate-800 dark:text-white flex items-center gap-2">
                                    <span className="flex items-center justify-center w-8 h-8 rounded-full bg-emerald-100 dark:bg-emerald-900 text-emerald-600 dark:text-emerald-300 text-sm font-bold shadow-sm border border-emerald-200 dark:border-emerald-800">4</span>
                                    Horarios
                                </h3>
                                <div className="flex items-center gap-3 bg-emerald-50 dark:bg-emerald-900/10 px-4 py-2 rounded-xl border border-emerald-100 dark:border-emerald-800/30">
                                    <Checkbox
                                        ...
                                        onChange={(e) => setFormData(prev => ({ ...prev, horariosFijos: e.target.checked }))}
                                        labelClassName="text-xs font-bold text-emerald-700 dark:text-emerald-300"
                                    />
                                    <span className="material-icons text-emerald-400 !text-sm group-hover:text-emerald-500 cursor-help" title="Si se marca, el estudiante no podrá elegir uno de los horarios, sino que se asume que debe asistir a todos.">info</span>
                                </div>
                            </div>
                            <div className="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-xl border border-slate-200 dark:border-slate-700">
                                {schedules.map((schedule, idx) => (
                                    <div key={idx} className="flex gap-2 mb-3 last:mb-0">
                                        ...
                                        />
                                        <button type="button" onClick={() => removeSchedule(idx)} className="p-2 hover:bg-rose-100 hover:text-rose-600 text-slate-400 rounded-lg transition-colors">
                                            <span className="material-icons !text-lg">delete</span>
                                        </button>
                                    </div>
                                ))}
                                <Button variant="secondary" onClick={addSchedule} type="button" className="mt-2 text-xs">
                                    + Agregar Horario
                                </Button>
                            </div>
                        </div>
                    ...
                    {showPreviewModal && (
                        <div className="fixed inset-0 z-[1500] bg-slate-950/90 backdrop-blur-md flex items-center justify-center p-4 md:p-6 animate-fade-in" onClick={() => setShowPreviewModal(false)}>
                            <div
                            ...
                            >
                                ...
                                <div className="p-5 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-white dark:bg-slate-900">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <h3 className="text-xl font-black text-slate-900 dark:text-white">Previsualización</h3>
                                            <p className="text-xs text-slate-500">Vista previa de la tarjeta y mensaje de WhatsApp</p>
                                        </div>
                                    </div>
                                    <button
                                        type="button"
                                        ...
                                        <div className="w-full">
                                            ...
                                            <div className="bg-slate-50 dark:bg-slate-950/50 rounded-2xl p-1 border border-slate-200 dark:border-slate-800 shadow-inner">
                                                <ConvocatoriaCardPremium
                                                    ...
                                                    timeline={{
                                                        inscripcion: (formData.fechaInicioInscripcion && formData.fechaFinInscripcion)
                                                    ...
                                                    invertLogo={selectedInstitution?.[FIELD_LOGO_INVERT_DARK_INSTITUCIONES] as boolean}
                                                    status="abierta"
                                                />
                                            </div>
                                        </div>
                                ...
                                <div className="p-5 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 flex justify-between items-center gap-4">
                                    <p className="text-xs text-slate-400 flex items-center gap-2">
                                        ...
                                        {formData.programarLanzamiento ? 'Se agendará para la fecha seleccionada.' : 'Se publicará inmediatamente.'}
                                    </p>
            ...
            <div className={activeTab === 'history' ? 'block' : 'hidden'}>
                <div className="mt-8 space-y-8">
                    {scheduledHistory.length > 0 && (
                        <div className="animate-fade-in">
                            ...
                            <div className="space-y-4">
                                {scheduledHistory.map(renderLaunchItem)}
                            </div>
                        </div>
                    )}
                    ...
                    <div>
                        ...
                        <div className="space-y-4">
                            {visibleHistory.length === 0 ? <EmptyState icon="history_edu" title="No hay convocatorias activas" message="Crea un nuevo lanzamiento." /> : visibleHistory.map(renderLaunchItem)}
                        </div>
                    </div>
