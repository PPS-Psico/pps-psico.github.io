  ...
  return (
    <div className="bg-white dark:bg-slate-800/70 p-4 rounded-xl border border-slate-200/80 dark:border-slate-700/80 shadow-sm transition-all duration-300 hover:shadow-md hover:border-slate-300 dark:hover:border-slate-600">
      ...
      <div className="mt-3 space-y-2 text-sm">
        <p className="text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-700/50 p-2 rounded-md font-medium">
          {event.schedule}
        </p>
        <div className="text-slate-600 dark:text-slate-400 flex items-start gap-2 pt-1">
          <span className="material-icons !text-base mt-0.5 text-slate-400 dark:text-slate-500">
            location_on
          </span>
          <div>
      ...
      {calendarLinks && (
        <div className="mt-3 pt-3 border-t border-slate-200/60 dark:border-slate-700/60 flex items-center gap-2 flex-wrap">
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-1.5 text-xs font-semibold text-slate-600 dark:text-slate-300 bg-white dark:bg-slate-700/50 border border-slate-300 dark:border-slate-600 rounded-lg px-2 py-1 hover:bg-slate-50 dark:hover:bg-slate-600/50 transition-colors"
          >
            <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
              <path
                d="M21 12.24C21 11.51 20.94 10.8 20.81 10.12H12.24V13.89H17.22C17.03 15.03 16.5 15.99 15.67 16.62V19.12H19.07C20.39 17.93 21.09 16.14 21.09 13.89C21.03 13.33 21 12.79 21 12.24Z"
                fill="#4285F4"
              />
              <path
                d="M12.24 21.49C15.03 21.49 17.34 20.59 19.07 19.12L15.67 16.62C14.79 17.24 13.59 17.62 12.24 17.62C9.64 17.62 7.42 15.99 6.63 13.71H3.12V16.29C4.86 19.41 8.25 21.49 12.24 21.49Z"
                fill="#34A853"
              />
              <path
                d="M6.63 13.71C6.42 13.11 6.3 12.48 6.3 11.84C6.3 11.2 6.42 10.57 6.63 9.97V7.39H3.12C2.42 8.76 2 10.25 2 11.84C2 13.43 2.42 14.92 3.12 16.29L6.63 13.71Z"
                fill="#FBBC05"
              />
              <path
                d="M12.24 6.05999C13.68 6.02999 15.09 6.56999 16.11 7.50999L19.14 4.71999C17.34 3.08999 14.97 2.18999 12.24 2.18999C8.25 2.18999 4.86 4.26999 3.12 7.38999L6.63 9.96999C7.42 7.68999 9.64 6.05999 12.24 6.05999Z"
                fill="#EA4335"
              />
            </svg>
            Google
          ...
          <a
            ...
            download={`${event.name.replace(/ /g, "_")}.ics`}
            className="inline-flex items-center gap-1.5 text-xs font-semibold text-slate-600 dark:text-slate-300 bg-white dark:bg-slate-700/50 border border-slate-300 dark:border-slate-600 rounded-lg px-2 py-1 hover:bg-slate-50 dark:hover:bg-slate-600/50 transition-colors"
          >
...
const HomeView: React.FC<HomeViewProps> = ({ myEnrollments, allLanzamientos, informeTasks }) => {
  const allPracticeEvents = useMemo(() => {
    ...
    const enrolledPractices = myEnrollments
      .filter(
        (e) =>
          normalizeStringForComparison(e[FIELD_ESTADO_INSCRIPCION_CONVOCATORIAS]) === "seleccionado"
      )
      .map((enrollment) => {
    ...
    enrolledPractices.forEach(({ pps, enrollment }) => {
      ...
      const scheduleDays = Object.keys(dayMap).filter(
        (d) => normalizedSchedule.includes(d) && !normalizedSchedule.includes(`no ${d}`)
      );
  ...
  const todayEvents = allPracticeEvents
  ...
  if (allPracticeEvents.length === 0 && pendingInformes.length === 0) {
    return (
      <EmptyState
        ...
        message="Cuando tengas prácticas o informes pendientes, aparecerán aquí."
      />
    );
  }
  ...
  return (
    <div className="space-y-8 animate-fade-in-up">
      ...
      <Card
        ...
        titleAs="h2"
        className="border-blue-200/80 dark:border-blue-700/80 bg-gradient-to-br from-blue-50/50 to-white dark:from-blue-900/20 dark:to-slate-800/20"
      >
        {todayEvents.length > 0 ? (
          <div className="space-y-4 mt-4 pt-4 border-t border-blue-200/50 dark:border-blue-700/50">
            {todayEvents.map((event, index) => (
              <UpcomingPracticeItem key={index} event={event} date={today} />
            ))}
          </div>
        ) : (
          <p className="mt-4 pt-4 border-t border-blue-200/50 dark:border-blue-700/50 text-slate-600 dark:text-slate-300">
            No tienes actividades programadas para hoy. ¡Aprovecha el día!
          </p>
        )}
      </Card>
      ...
      {pendingInformes.length > 0 && (
        <Card
          ...
          title="Informes con Entrega Próxima"
          titleAs="h2"
          className="border-amber-200/80 dark:border-amber-700/80 bg-gradient-to-br from-amber-50/50 to-white dark:from-amber-900/20 dark:to-slate-800/20"
        >
          <div className="space-y-3 mt-4 pt-4 border-t border-amber-200/50 dark:border-amber-700/50">
            {pendingInformes.map((task) => (
              <a
                key={task.convocatoriaId}
                href={task.informeLink}
                target="_blank"
                rel="noopener noreferrer"
                className="block p-3 rounded-lg bg-white/50 dark:bg-slate-800/50 border border-amber-200/60 dark:border-amber-700/60 hover:bg-amber-50/50 dark:hover:bg-amber-900/30 transition-colors"
              >
                ...
                <p className="text-xs text-amber-700 dark:text-amber-300 font-semibold mt-1">
                  ...
                  {formatDate(
                    new Date(
                      parseToUTCDate(task.fechaFinalizacion)!.getTime() + 30 * 24 * 60 * 60 * 1000
                    ).toISOString()
                  )}
                </p>
              </a>
