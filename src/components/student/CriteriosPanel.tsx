...
const CriteriosPanel: React.FC<CriteriosPanelProps> = ({
  ...
  const requisitosCursadaCumplidos = useMemo(
    () =>
  ...
  // Celebración: confetti UFLO una sola vez por sesión cuando se cumplen los
  // tres requisitos cuantitativos de cursada. No implica acreditación final.
  const celebratedRef = useRef(false);
  useEffect(() => {
    if (!requisitosCursadaCumplidos || celebratedRef.current) return;
        ...
        <span className="text-sm font-medium text-slate-600 dark:text-slate-400">
          {horasFaltantes > 0 ? (
            <>
              ...
              para completar el requisito de horas.
            </>
          ) : (
            <b className="text-uflo-teal dark:text-[#5EE6B7] font-semibold">
          ...
          {requisitosCursadaCumplidos ? (
            <div className="w-full text-center py-3 bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-800/40 rounded-xl">
              <p className="text-sm font-semibold text-emerald-600 dark:text-emerald-400 flex items-center justify-center gap-1.5">
                ...
                Requisitos de cursada completos
              </p>
              <p className="mt-1 text-xs text-emerald-800/70 dark:text-emerald-200/70">
                Cumpliste horas, rotación y orientación.
              </p>
            </div>
          ) : (
