...
const WelcomeBanner: React.FC<WelcomeBannerProps> = ({ studentName, isLoading }) => {
  ...
  return (
    <div
      className={`welcome-reveal relative mb-8 pt-4 px-2 ${isLoading ? "" : "is-revealed"}`}
      aria-busy={isLoading}
    >
      <div className="welcome-reveal__skeleton" aria-hidden="true">
        <div className="flex flex-col md:flex-row md:items-baseline justify-between gap-4">
          <div className="flex-1 space-y-4">
            <SkeletonBar className="h-12 sm:h-16 md:h-20 w-3/4" />
            <SkeletonBar className="h-8 sm:h-10 w-1/2" />
          </div>
      ...
      <div className="welcome-reveal__content" aria-hidden={isLoading}>
        <div className="flex flex-col md:flex-row md:items-baseline justify-between gap-4">
        ...
        <div className="hidden md:block mt-6 h-px w-full bg-gradient-to-r from-slate-200 via-slate-100 to-transparent dark:from-slate-800 dark:via-slate-900" />
      </div>
    </div>
  );
};
...
const SkeletonBar: React.FC<{ className: string }> = ({ className }) => (
  <div
    className={`${className} bg-slate-200 dark:bg-slate-700 rounded-xl relative overflow-hidden`}
  >
