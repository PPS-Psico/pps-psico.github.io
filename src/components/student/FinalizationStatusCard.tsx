...
const FinalizationStatusCard: React.FC<FinalizationStatusCardProps> = ({ status, requestDate, studentName }) => {
                    ...
                    <div className="relative z-10 flex flex-col items-center gap-8">
                        ...
                        <div className="mt-6">
                            ...
                            >
                                ...
                                <span className="material-icons !text-2xl group-hover:translate-x-1 transition-transform">open_in_new</span>
                            </a>
                        </div>
                    </div>
                </div>
    ...
    if (isEnProceso) {
        ...
        bannerText = "Tus documentos fueron validados correctamente y el expediente se encuentra en el circuito de acreditación interna.";
    ...
    const steps = [
        { title: "Validación Documental", desc: "Revisión de firmas y planillas.", icon: "inventory_2" },
        { title: "Circuito Administrativo", desc: "Aprobación final por áreas UFLO.", icon: "settings_suggest" },
                            ...
                            {steps.map((step, idx) => {
                                ...
                                return (
                                    <div key={idx} className={`relative flex items-center gap-8 ${isPending ? 'opacity-50 grayscale' : 'opacity-100'}`}>
                                        ...
                                        `}>
                                            <span className={`material-icons !text-2xl ${isActive ? 'animate-pulse' : ''}`}>{isCompleted ? 'check' : step.icon}</span>
                                        </div>
                                        <div>
                                            <div className="flex items-center gap-3 mb-1">
                                                <h4 className={`text-xl font-black tracking-tight ${isActive ? 'text-blue-700 dark:text-blue-400' : isCompleted ? 'text-emerald-700 dark:text-emerald-400' : 'text-slate-500'}`}>{step.title}</h4>
                                                {isCompleted && <span className="text-[9px] bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-md font-black uppercase tracking-widest">OK</span>}
                                                {isActive && <span className="text-[9px] bg-blue-100 text-blue-700 px-2 py-0.5 rounded-md font-black uppercase tracking-widest animate-pulse">EN CURSO</span>}
                                            </div>
                                            <p className="text-slate-500 dark:text-slate-400 text-base font-medium leading-relaxed max-w-md">{step.desc}</p>
                                        </div>
                                    </div>
                                );
                            })}
                ...
                <div className="col-span-1 lg:col-span-4 flex flex-col gap-6">
                    ...
                    <div className="glass-panel rounded-[3rem] shadow-xl overflow-hidden flex flex-col p-10">
                        ...
                        <div className="flex flex-col items-center justify-center mb-8">
                            <span className={`text-8xl font-black tracking-tighter leading-none ${isOverdue ? "text-rose-500" : isPaused ? "text-amber-500" : "text-slate-900 dark:text-white"}`}>
                        ...
                        <div className="flex justify-between items-center text-center">
                            <div className="flex-1 border-r border-slate-100 dark:border-slate-800"><p className="text-[10px] font-black text-slate-400 uppercase mb-1">Enviado</p><p className="text-sm font-bold text-slate-700 dark:text-slate-300 font-mono">{formatDate(startDate.toISOString())}</p></div>
                            <div className="flex-1"><p className="text-[10px] font-black text-slate-400 uppercase mb-1">Estimado</p><p className={`text-sm font-bold font-mono ${isOverdue ? 'text-rose-500' : 'text-slate-700 dark:text-slate-300'}`}>{formatDate(targetDate.toISOString())}</p></div>
                        </div>
                    </div>
