...
const WhatsAppExportButton: React.FC<WhatsAppExportButtonProps> = ({
  practicas,
  criterios,
  isLoading,
  selectedOrientacion,
  ...
  const handleExport = () => {
    if (isLoading || !criterios || !practicas || !studentNameForPanel || !studentDetails) {
      showModal(
        "Error",
        "Los datos del estudiante aún se están cargando. Inténtalo de nuevo en un momento."
      );
      return;
    }
  ...
  return (
    <button
      ...
      className="fixed bottom-6 right-6 z-50 w-14 h-14 bg-blue-600 text-white rounded-full shadow-lg flex items-center justify-center
                 transition duration-300 ease-in-out transform hover:scale-110 hover:shadow-xl focus:outline-none focus:ring-4 focus:ring-blue-300"
