...
const StudentDiagnostics: React.FC = () => {
  ...
  return (
    <Card
      title="Diagnóstico de Estudiante"
      icon="troubleshoot"
      className="border-amber-200 dark:border-amber-800 bg-amber-50/30 dark:bg-amber-900/10"
    >
      <div className="flex gap-4 items-end">
        ...
        <Button onClick={handleCheck} isLoading={isLoading} icon="search">
          Diagnosticar
        </Button>
      </div>
      ...
      {result && (
        <div className="mt-6 space-y-4 animate-fade-in">
          <div
            className={`p-4 rounded-lg border ${result.student.count === 1 ? "bg-emerald-50 border-emerald-200 text-emerald-800" : "bg-rose-50 border-rose-200 text-rose-800"}`}
          >
            <h4 className="font-bold flex items-center gap-2">
              <span className="material-icons">
                {result.student.count === 1 ? "check_circle" : "warning"}
              </span>
              Estado del Registro
            </h4>
            <p className="text-sm mt-1">
              {result.student.count === 0
                ? "No se encontró el estudiante en la base de datos."
                : result.student.count > 1
                  ? `⚠️ ALERTA: Se encontraron ${result.student.count} registros duplicados para este legajo. Esto causa errores en la inscripción.`
                ...
                <div>
                  ...
                  {s[FIELD_USER_ID_ESTUDIANTES] ? (
                    <span className="text-emerald-600 font-bold flex items-center gap-1">
                      <span className="material-icons !text-sm">link</span> Sí
                    </span>
                  ) : (
                    <span className="text-rose-600 font-bold flex items-center gap-1">
                      <span className="material-icons !text-sm">link_off</span> No (Causa de error)
                    </span>
                  )}
                </div>
              </div>
