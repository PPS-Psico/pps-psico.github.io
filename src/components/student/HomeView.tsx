...
const HomeView: React.FC<HomeViewProps> = ({
    ...
    const renderCard = (lanzamientoWithSuffix: LanzamientoPPS & { key_suffix?: string }) => {
        ...
        const logoData = (institutionLogoMap && (institutionLogoMap.get(normalizeStringForComparison(ppsName)) || institutionLogoMap.get(normalizeStringForComparison(groupName))));

        // Check if this PPS was already completed by the student
        const normalizedPpsName = normalizeStringForComparison(ppsName);
        const isCompleted = completedLanzamientoIds.has(lanzamiento.id) || completedLanzamientoIds.has(normalizedPpsName);
        ...
        return (
            <ConvocatoriaCardPremium
                ...
                actividades={Array.isArray(lanzamiento.actividades_lista) ? lanzamiento.actividades_lista : (lanzamiento.actividades_lista ? [String(lanzamiento.actividades_lista)] : [])}
    ...
    return (
        <div className="space-y-12 animate-fade-in">
            ...
            {closedLanzamientos.length > 0 ? (
                <div className="pt-4">
                    <h3 className="text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-6 flex items-center gap-2 pl-1">
                        <span className="w-1.5 h-1.5 rounded-full bg-slate-400"></span> Convocatorias Finalizadas
                    </h3>
