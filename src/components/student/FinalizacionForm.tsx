...
const FinalizacionForm: React.FC<FinalizacionFormProps> = ({
  ...
  const submitMutation = useMutation({
    mutationFn: async () => {
      ...
      if (fileCategories.horas.files.length === 0)
        throw new Error("Falta la Planilla de Seguimiento.");
      if (fileCategories.asistencia.files.length === 0)
        throw new Error("Falta la Planilla de Asistencia.");
  ...
  if (isSubmitted) {
    return createPortal(
      ...
      >
        <div className="relative w-full max-w-2xl max-h-[90vh] bg-white dark:bg-slate-900 rounded-2xl shadow-2xl flex flex-col border border-slate-200 dark:border-slate-800 overflow-hidden">
          <Card
  ...
  return createPortal(
    ...
    >
      <div className="relative w-full max-w-2xl max-h-[90vh] bg-white dark:bg-slate-900 rounded-2xl shadow-2xl flex flex-col border border-slate-200 dark:border-slate-800 overflow-hidden">
        {/* Header Estilizado */}
        <div className="flex-shrink-0 p-4 sm:p-5 border-b border-indigo-100 dark:border-slate-800 bg-indigo-50/50 dark:bg-slate-900/50 flex justify-between items-start">
          <div className="flex items-center gap-3 sm:gap-4">
            <div className="p-2 sm:p-3 bg-white dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 rounded-2xl shadow-sm border border-indigo-100 dark:border-slate-700">
              <span className="material-icons !text-xl sm:!text-2xl">verified</span>
            </div>
            <div>
              ...
              <p className="text-xs sm:text-sm font-medium text-slate-500 dark:text-slate-400">
                Adjunta la documentación para cerrar el ciclo.
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 sm:space-y-6 custom-scrollbar">
            ...
            return (
              ...
              >
                <div className="p-4 sm:p-5">
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex items-center gap-2 sm:gap-3">
                      <div className={`p-2 rounded-xl ${section.bgColor} ${section.iconColor}`}>
                        <span className="material-icons !text-lg sm:!text-xl">{section.icon}</span>
                      </div>
                      <div>
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400 mb-3 sm:mb-4 pl-1">
                    {section.desc}
                  </p>
                  ...
                  <div
                    className={`
                      rounded-xl border-2 border-dashed p-3 sm:p-4 text-center cursor-pointer transition-colors
                      ${
                        isActive
                  ...
                  >
                    ...
                    <div className="flex flex-col items-center gap-1">
                      <span
                        className={`material-icons !text-xl sm:!text-2xl ${isActive ? "text-blue-500" : "text-slate-300 dark:text-slate-600"}`}
                      >
                        cloud_upload
                      </span>
                  ...
                  {hasFiles && (
                    <div className="mt-2 sm:mt-3 space-y-2">
                      {categoryState.files.map((file, idx) => (
                        <div
                          key={idx}
                          className="flex items-center justify-between p-2 bg-slate-50 dark:bg-slate-900/50 rounded-lg border border-slate-100 dark:border-slate-700/50"
                        >
                          <div className="flex items-center gap-2 overflow-hidden">
                            <span className="material-icons text-slate-400 !text-base shrink-0">
                              description
                            </span>
          ...
          <div className="p-3 sm:p-4 bg-amber-50 dark:bg-amber-900/10 rounded-2xl border border-amber-100 dark:border-amber-800/50">
        ...
        <div className="flex-shrink-0 p-4 sm:p-5 border-t border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/50 flex flex-col sm:flex-row justify-end items-center gap-2 sm:gap-3">
          {submitMutation.isPending && (
            <span className="text-xs font-bold text-slate-500 animate-pulse flex items-center gap-1">
              <div className="w-2 h-2 bg-blue-500 rounded-full animate-bounce"></div> Subiendo
              archivos...
            </span>
          )}
          <div className="flex gap-2 sm:gap-3 w-full sm:w-auto">
            ...
            <Button
              ...
              disabled={
                fileCategories.horas.files.length === 0 ||
                fileCategories.asistencia.files.length === 0 ||
                fileCategories.informe.files.length === 0
              }
              icon="send"
