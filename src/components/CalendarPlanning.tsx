...
const CalendarPlanning: React.FC<CalendarPlanningProps> = ({ items }) => {
  ...
  const calendarData = useMemo(() => {
    ...
    items.forEach((item) => {
      ...
      const dateStr =
        item[FIELD_FECHA_RELANZAMIENTO_LANZAMIENTOS] || item[FIELD_FECHA_INICIO_LANZAMIENTOS];
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 animate-fade-in">
      {MONTHS.map((monthName, index) => {
        ...
        return (
          ...
          >
            <div
              className={`p-3 border-b ${hasLaunches ? "bg-indigo-50/50 dark:bg-indigo-900/20 border-indigo-100 dark:border-indigo-800" : "border-slate-100 dark:border-slate-800"} flex justify-between items-center`}
            >
              <h3
                className={`font-bold ${hasLaunches ? "text-indigo-900 dark:text-indigo-200" : "text-slate-500 dark:text-slate-500"}`}
              >
                {monthName}{" "}
                {isCurrentMonth && (
                  <span className="text-[10px] text-blue-500 uppercase ml-1">(Actual)</span>
                )}
              </h3>
              {hasLaunches && (
              ...
              {hasLaunches ? (
                <ul className="space-y-2">
                  {launches.map((launch) => (
                    <li
                      key={launch.id}
                      className="text-xs group relative pl-3 border-l-2 border-emerald-400"
                    >
                      <div
                      ...
                      <div className="text-slate-500 dark:text-slate-400 flex justify-between mt-0.5">
                        <span>
                          {(launch as any)._displayDay !== "?"
                            ? `Día ${(launch as any)._displayDay}`
                            : "A confirmar"}
                        </span>
                        {launch[FIELD_CUPOS_DISPONIBLES_LANZAMIENTOS] && (
                          <span>{launch[FIELD_CUPOS_DISPONIBLES_LANZAMIENTOS]} cupos</span>
                        )}
                      </div>
                    </li>
                  ))}
