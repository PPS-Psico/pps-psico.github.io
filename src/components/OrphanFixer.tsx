...
const OrphanFixer: React.FC = () => {
    ...
    try {
      ...
      const validLaunchIds = new Set(
        launches
          ?.filter(
            (l) =>
              parseToUTCDate(l[FIELD_FECHA_INICIO_LANZAMIENTOS])?.getUTCFullYear() === currentYear
          )
          .map((l) => l.id)
      );
      ...
      const [convRes, pracRes] = await Promise.all([
        ...
        supabase
          ...
          .select(FIELD_ESTUDIANTE_LINK_PRACTICAS + ",fecha_inicio"),
      ]);
      ...
      const toFix =
        students
          ?.filter((s) => s.estado !== "Activo" && s.estado !== "Finalizado")
          .map((s) => s.id) || [];
  ...
  const fixStatuses = async () => {
    ...
    try {
      ...
      setToast({
        m: `¡Éxito! Se actualizaron ${idsToFix.length} estudiantes a estado 'Activo'.`,
        t: "success",
      });
    ...
    <Card title="Sincronizador de Estados (Métricas)" icon="sync_alt">
      ...
      <div className="p-4 space-y-4">
        <p className="text-sm text-slate-600 dark:text-slate-400">
          Esta herramienta detecta alumnos que tienen trámites en {new Date().getFullYear()} pero
          figuran como 'Inactivos' en la base de datos.
        </p>
        ...
        {isScanning ? (
          ...
          <div className="space-y-4">
            ...
            <button
              onClick={fixStatuses}
              disabled={isFixing}
              className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 rounded-xl shadow-lg flex items-center justify-center gap-2 transition"
            >
              {isFixing ? (
