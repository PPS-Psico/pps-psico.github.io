...
const MOCK_TIMELINE_DATA: any[] = [
  {
    id: "tl_mock_1",
    [FIELD_NOMBRE_PPS_LANZAMIENTOS]: "Hospital de Simulación",
    [FIELD_FECHA_INICIO_LANZAMIENTOS]: `${new Date().getFullYear()}-03-10`,
    [FIELD_CUPOS_DISPONIBLES_LANZAMIENTOS]: 10,
  },
  {
    id: "tl_mock_2",
    [FIELD_NOMBRE_PPS_LANZAMIENTOS]: "Escuela de Prueba",
    [FIELD_FECHA_INICIO_LANZAMIENTOS]: `${new Date().getFullYear()}-03-20`,
    [FIELD_CUPOS_DISPONIBLES_LANZAMIENTOS]: 5,
  },
  {
    id: "tl_mock_3",
    [FIELD_NOMBRE_PPS_LANZAMIENTOS]: "Empresa Ficticia",
    [FIELD_FECHA_INICIO_LANZAMIENTOS]: `${new Date().getFullYear()}-05-05`,
    [FIELD_CUPOS_DISPONIBLES_LANZAMIENTOS]: 8,
  },
  {
    id: "tl_mock_4",
    [FIELD_NOMBRE_PPS_LANZAMIENTOS]: "Hospital de Simulación - Tarde",
    [FIELD_FECHA_INICIO_LANZAMIENTOS]: `${new Date().getFullYear()}-03-12`,
    [FIELD_CUPOS_DISPONIBLES_LANZAMIENTOS]: 8,
  },
];
...
const TimelineView: React.FC<TimelineViewProps> = ({ isTestingMode = false }) => {
  ...
  const {
    ...
    queryFn: () =>
      isTestingMode ? Promise.resolve(MOCK_TIMELINE_DATA as LanzamientoPPS[]) : fetchTimelineData(),
  });
    ...
    <div className="animate-fade-in-up space-y-8">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="relative w-full sm:w-48">
          ...
          <span className="absolute right-3 top-1/2 -translate-y-1/2 material-icons text-slate-400 dark:text-slate-500 pointer-events-none">
            expand_more
          </span>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-[#0B1120] p-6 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden">
        ...
        <div className="relative z-10 grid grid-cols-1 sm:grid-cols-2 gap-8 divide-y sm:divide-y-0 sm:divide-x divide-slate-100 dark:divide-slate-800 text-center">
          ...
          <div className="pt-4 sm:pt-0">
            ...
            <p className="text-6xl font-black text-indigo-600 dark:text-indigo-400 tracking-tighter">
              {totalCuposForYear}
            </p>
          </div>
        </div>
      ...
      {launchesByMonth.length === 0 ? (
        <EmptyState
          icon="calendar_today"
          title={`Sin Actividad en ${targetYear}`}
          message={`No se encontraron lanzamientos de PPS para el ciclo ${targetYear}.`}
        />
      ) : (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                  {monthData.institutions.map((inst) => {
                    ...
                    return (
                      <div
                        key={inst.name}
                        className="flex flex-col justify-center p-3 rounded-xl bg-slate-50/50 dark:bg-slate-800/30 border border-slate-100 dark:border-slate-700/50 hover:bg-white dark:hover:bg-slate-800 transition-colors"
                      >
                        <div className="flex justify-between items-start gap-2">
                          <span className="font-bold text-sm text-slate-700 dark:text-slate-200 leading-tight">
                            {inst.name}
                          </span>
