...
const ChangePasswordModal: React.FC<ChangePasswordModalProps> = ({ isOpen }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Intentamos primero por RPC (más seguro/admin). La RPC no recibe args.
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md p-8 border border-slate-200 dark:border-slate-700">
        <div className="text-center mb-6">
          ...
          <p className="text-slate-600 dark:text-slate-400 mt-2 text-sm">
            Por seguridad, necesitamos que definas una nueva contraseña personal para unificar el
            acceso a tu panel.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg shadow-md transition disabled:opacity-50 flex items-center justify-center gap-2"
          >
            {isLoading ? (
              <>
