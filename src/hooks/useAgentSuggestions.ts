...
export function useAgentSuggestions(isTestingMode = false) {
  ...
  const { data: dailyBrief, isLoading: isBriefLoading } = useQuery<DailyBrief | null>({
    ...
    queryFn: async () => {
      ...
      if (error) {
        console.error("[useAgentSuggestions] Error fetching daily brief:", error);
        return null;
      }
      if (!data) return null;
  ...
  const { data: emailDrafts = [], isLoading: isDraftsLoading } = useQuery<EmailDraft[]>({
    ...
    queryFn: async () => {
      ...
      if (error) {
        console.error("[useAgentSuggestions] Error fetching email drafts:", error);
        return [];
      }
  ...
  const approveDraft = useMutation({
    mutationFn: async ({ id, editedText }: { id: string; editedText?: string }) => {
      const updatePayload: Record<string, unknown> = { estado: "approved" };
