...
import { 
    ...
    FIELD_LEGAJO_PPS,
    FIELD_ESTADO_PPS as FIELD_ESTADO_PPS_CONST,
...
import PreAcreditacionCheckModal from '../components/PreAcreditacionCheckModal';
import { normalizeStringForComparison } from '../utils/formatters';
import { fetchAllData } from '../services/supabaseService';
import { institucionArraySchema } from '../schemas';
...
export const StudentHome: React.FC = () => {
    ...
    const handleProceedToFinalForm = useCallback(() => {
        setIsPreCheckOpen(false);
        setIsFinalizationModalOpen(true);
    }, []);

    // IF FINALIZATION IS ACTIVE, RETURN STATUS CARD IMMEDIATELY (Full Page Replacement)
        ...
        return (
             <ErrorBoundary>
                <FinalizationStatusCard 
                    ...
                    studentName={studentDetails?.[FIELD_NOMBRE_ESTUDIANTES] || 'Estudiante'}
                />
            </ErrorBoundary>
        );
    }
    ...
    return (
        <ErrorBoundary>
             <PreAcreditacionCheckModal 
                isOpen={isPreCheckOpen}
                onClose={() => setIsPreCheckOpen(false)}
                onContinue={handleProceedToFinalForm}
             />
            ...
            <HomeView 
                ...
                finalizacionRequest={finalizacionRequest} // Passed to HomeView
            />
        </ErrorBoundary>
    );
};
...
const StudentDashboard: React.FC<StudentDashboardProps> = ({ user, activeTab, onTabChange, showExportButton = false }) => {
  ...
  const [isFinalizationModalOpen, setIsFinalizationModalOpen] = useState(false);
  const [isPreCheckOpen, setIsPreCheckOpen] = useState(false);
  const [isPreSolicitudOpen, setIsPreSolicitudOpen] = useState(false);
  ...
  // --- Logic for Existing Institutions List ---
  // Independent query to fetch ALL institutions names directly from DB to ensure completeness
  const { data: allInstitutionsNames = [] } = useQuery({
      queryKey: ['allInstitutionsForCheck'],
      queryFn: async () => {
          const { records } = await fetchAllData<any>(
              TABLE_NAME_INSTITUCIONES,
          ...
          records.forEach(r => {
              const name = r[FIELD_NOMBRE_INSTITUCIONES];
              if (name) {
  ...
  // Fallback to local data if query hasn't run yet, though independent query is preferred
  const existingInstitutions = useMemo(() => {
      if (allInstitutionsNames.length > 0) return allInstitutionsNames;
  ...
  }, [allLanzamientos, allInstitutionsNames]);
  ...
  const createSolicitudMutation = useMutation({
      mutationFn: async (formData: any) => {
          ...
          const newRecord: Partial<SolicitudPPSFields> = {
              ...
              [FIELD_ESTADO_PPS_CONST]: 'Pendiente',
  ...
  const handleStartSolicitud = useCallback(() => {
      setIsPreSolicitudOpen(true);
  }, []);

  const handleProceedToSolicitudForm = useCallback(() => {
      setIsPreSolicitudOpen(false);
  ...
  const homeContent = useMemo(() => (
    <ErrorBoundary>
        <HomeView 
            ...
            criterios={criterios}
            onOpenFinalization={handleOpenFinalization}
            finalizacionRequest={finalizacionRequest} // Pass request to hide button
        />
    </ErrorBoundary>
  ), [enrollmentMap, allLanzamientos, informeTasks, lanzamientos, studentDetails, enrollStudent.mutate, institutionAddressMap, completedLanzamientoIds, criterios, handleOpenFinalization, finalizacionRequest]);
  ...
  const studentDataTabs = useMemo(() => {
    const tabs: { id: TabId; label: string; icon: string; content: React.ReactNode; badge?: number }[] = [
      ...
      // Ocultar pestaña informes explícita, ahora se muestra en Inicio
  ...
  // --- SPECIAL VIEW: FINALIZATION IN PROGRESS ---
  ...
  return (
    <>
      ...
      {/* Modal de Pre-Check Acreditación */}
      <PreAcreditacionCheckModal 
        isOpen={isPreCheckOpen}
        onClose={() => setIsPreCheckOpen(false)}
        onContinue={handleProceedToFinalForm}
      ...
      {/* Modal de Pre-Check Solicitud Nueva (Buscador Instituciones) */}
      <PreSolicitudCheckModal 
        isOpen={isPreSolicitudOpen}
        onClose={() => setIsPreSolicitudOpen(false)}
        onContinue={handleProceedToSolicitudForm}
      ...
      <div className="md:hidden no-print space-y-8 animate-fade-in-up">
          {currentActiveTab === 'inicio' && (
              <>
                  <div className="relative p-6 rounded-3xl border border-slate-200/80 dark:border-slate-700/80 shadow-lg overflow-hidden bg-gradient-to-br from-blue-50/80 via-white/70 to-slate-50/80 dark:from-blue-900/30 dark:via-slate-900/20 dark:to-black/30 backdrop-blur-lg mb-6">
