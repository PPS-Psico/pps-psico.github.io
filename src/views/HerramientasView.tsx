...
const HerramientasView: React.FC<HerramientasViewProps> = ({ onStudentSelect }) => {
  ...
  return (
    <>
      <SubTabs
        tabs={herramientasSubTabs}
        activeTabId={activeHerramientasTabId}
        onTabChange={setActiveHerramientasTabId}
      />
      <div className="mt-6">
        {activeHerramientasTabId === "penalizaciones" && <PenalizationManager />}
        {activeHerramientasTabId === "search" && (
          <div className="p-4">
            <AdminSearch onStudentSelect={onStudentSelect} />
          </div>
        )}
