...
export const trackAcademicEvents = {
  ...
  studentRegistration: (studentData: { legajo: string; orientation: string }) => {
    trackEvent('student_registered', 'academic', `${studentData.legajo} - ${studentData.orientation}`);
  },

  // Practice submission
  practiceSubmission: (practiceData: { hours: number; institution: string; type: string }) => {
    trackEvent('practice_submitted', 'academic', `${practiceData.institution} - ${practiceData.type}`, practiceData.hours);
  },

  // Convocatoria application
  convocatoriaApplication: (convocatoriaData: { name: string; orientation: string }) => {
    trackEvent('convocatoria_applied', 'academic', `${convocatoriaData.name} - ${convocatoriaData.orientation}`);
  },
...
export const setCustomDimensions = (userData: { role?: string; orientation?: string; legajo?: string }) => {
  if (import.meta.env.PROD) {
    ...
    if (userData.orientation) {
      ReactGA.gtag('event', 'student_orientation_dimension', { student_orientation: userData.orientation });
    }
    if (userData.legajo) {
