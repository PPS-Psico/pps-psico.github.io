...
export const toggleStudentSelection = async (
  ...
  try {
    ...
    await db.convocatorias.update(
      convocatoriaId,
      updatePayload as Parameters<typeof db.convocatorias.update>[1]
    );
    ...
    if (isSelecting) {
      ...
      if (!existing) {
        ...
        const existingPractica = existing as { id: string; [key: string]: unknown };
